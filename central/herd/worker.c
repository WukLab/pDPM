...
void* run_worker(void* arg) {
  ...
  for (i = 0; i < num_server_ports; i++) {
    ...
    cb[i] = hrd_ctrl_blk_init(wrkr_lid,          /* local_hid */
                              ...
                              0, 0,              /* #conn qps, uc */
                              NULL, 40960, -1, /*prealloc conn buf, buf size, key */
                              NUM_UD_QPS, 4096,
        ...
        //setup remote memory
        {
                ...
                for (j = 0; j < NUM_MEMORY; j++) 
                {
                        ...
                        mr_qp = mr_qp_list[j];
                        backupmr_qp_list[j] = malloc(sizeof(struct hrd_qp_attr *) * HERD_NUM_KEYS);
                        ...
                        for(per_mr=0;per_mr<=HERD_NUM_KEYS;per_mr++)
                        {
                                ...
                                else
                                {
                                        ...
                                        mr_qp[per_mr]->buf_addr = mr_qp[per_mr-1]->buf_addr + HERD_VALUE_SIZE;
                                        backupmr_qp[per_mr]->buf_addr = backupmr_qp[per_mr-1]->buf_addr + HERD_VALUE_SIZE;
                                }

                                if(per_mr%50000 == 0)
                                {
                                        printf("main: worker %d get memory %d of %d mr from %d. \n", wrkr_lid, per_mr, HERD_NUM_KEYS, j);
                                }
                        }
                        printf("main: worker %d get memory %d of %d mr. \n", wrkr_lid, j, HERD_NUM_KEYS);
                }
        }
  }
        /*
        {
                ...
                test_mr = ibv_reg_mr(mem_cb[0]->pd, test_region, 1024, IBV_ACCESS_LOCAL_WRITE|IBV_ACCESS_REMOTE_READ|IBV_ACCESS_REMOTE_WRITE);
                test_sge.length = 24;
                ...
                printf("worker %d %s %llx %lx\n", wrkr_lid, (char *)test_region, (unsigned long long int)wr.wr.rdma.remote_addr, (unsigned long int)wr.wr.rdma.rkey);
  ...
  long long recv_traffic = 0;
  long long send_traffic = 0;
  ...
  struct ibv_mr **mitsume_mr_list = malloc(sizeof(struct ibv_mr *) * MICA_MAX_BATCH_SIZE);
  for(i=0;i<MICA_MAX_BATCH_SIZE;i++)
  {
        mitsume_space_list[i] = memalign(4096, sizeof(struct mica_op));
        mitsume_mr_list[i] = ibv_reg_mr(mem_cb[0]->pd, mitsume_space_list[i], sizeof(struct mica_op), IBV_ACCESS_LOCAL_WRITE|IBV_ACCESS_REMOTE_WRITE|IBV_ACCESS_REMOTE_READ);
  }
  ...
  while (1) {
    if (unlikely(rolling_iter >= M_1)) {
      ...
      printf(
          ...
          "send %.2f recv %.2f\n",
          wrkr_lid, M_1 / seconds, (double)nb_tx_tot / nb_post_send,
          (double)kv.num_get_fail / kv.num_get_op,
          (double)send_traffic/seconds, (double)recv_traffic/seconds);
    ...
    mica_batch_op(&kv, wr_i, op_ptr_arr, resp_arr, mem_cb, mitsume_space_list, mitsume_mr_list, mr_qp_list, backupmr_qp_list, wrkr_lid, &send_traffic, &recv_traffic);
    ...
    for (wr_i = 0; wr_i < nb_new_req_tot; wr_i++) {
      ...
      if(op_ptr_arr[wr_i]->opcode == MICA_OP_GET)
      {
         ...
         send_traffic = send_traffic + HERD_VALUE_SIZE;
      }
      else
      {
         sgl[wr_i].length = 0;
