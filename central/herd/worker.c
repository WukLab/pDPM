  ...
  for (i = 0; i < num_server_ports; i++) {
    ...
    // setup remote memory
    {
      ...
      mem_cb[i] = hrd_ctrl_blk_init(
          ...
          NULL, 40960, -1, 0, 0, -1); /* #dgram qps, buf size, shm key */
        ...
        for (per_mr = 0; per_mr <= HERD_NUM_KEYS; per_mr++) {
          ...
          if (per_mr == 0) {
            ...
            mr_qp[per_mr]->buf_addr =
                mr_qp[per_mr - 1]->buf_addr + HERD_VALUE_SIZE;
            backupmr_qp[per_mr]->buf_addr =
                backupmr_qp[per_mr - 1]->buf_addr + HERD_VALUE_SIZE;
          }
