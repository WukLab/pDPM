#include "mica.h"
#include "hrd.h"

#include "pthread.h"
#define MICA_MAX_KEYS 100000
pthread_rwlock_t rwlock[MICA_MAX_KEYS];

pthread_rwlock_t queue_lock[MICA_BUCKET];
int queue_setmap[MICA_MAX_KEYS];


// C program for array implementation of queue 
#include <stdio.h> 
#include <stdlib.h> 
#include <limits.h> 
  
// A structure to represent a queue 
struct Queue 
{ 
    int front, rear, size; 
    unsigned capacity; 
    int* array; 
}; 
struct Queue *MITSUME_QUEUE[MICA_BUCKET];
  
// function to create a queue of given capacity.  
// It initializes size of queue as 0 
struct Queue* createQueue(unsigned capacity) 
{ 
    struct Queue* queue = (struct Queue*) malloc(sizeof(struct Queue)); 
    queue->capacity = capacity; 
    queue->front = queue->size = 0;  
    queue->rear = capacity - 1;  // This is important, see the enqueue 
    queue->array = (int*) malloc(queue->capacity * sizeof(int)); 
    return queue; 
} 
  
// Queue is full when size becomes equal to the capacity  
int isFull(struct Queue* queue) 
{  return (queue->size == queue->capacity);  } 
  
// Queue is empty when size is 0 
int isEmpty(struct Queue* queue) 
{  return (queue->size == 0); } 
  
// Function to add an item to the queue.   
// It changes rear and size 
void enqueue(struct Queue* queue, int item) 
{ 
    if (isFull(queue)) 
        return; 
    queue->rear = (queue->rear + 1)%queue->capacity; 
    queue->array[queue->rear] = item; 
    queue->size = queue->size + 1; 
} 
  
// Function to remove an item from queue.  
// It changes front and size 
int dequeue(struct Queue* queue) 
{ 
    if (isEmpty(queue)) 
        return INT_MIN; 
    int item = queue->array[queue->front]; 
    queue->front = (queue->front + 1)%queue->capacity; 
    queue->size = queue->size - 1; 
    return item; 
} 
  
// Function to get front of queue 
int front(struct Queue* queue) 
{ 
    if (isEmpty(queue)) 
        return INT_MIN; 
    return queue->array[queue->front]; 
} 
  
// Function to get rear of queue 
int rear(struct Queue* queue) 
{ 
    if (isEmpty(queue)) 
        return INT_MIN; 
    return queue->array[queue->rear]; 
} 



int is_power_of_2(int x) {
  return (x == 1 || x == 2 || x == 4 || x == 8 || x == 16 || x == 32 ||
          x == 64 || x == 128 || x == 256 || x == 512 || x == 1024 ||
          x == 2048 || x == 4096 || x == 8192 || x == 16384 || x == 32768 ||
          x == 65536 || x == 131072 || x == 262144 || x == 524288 ||
          x == 1048576 || x == 2097152 || x == 4194304 || x == 8388608 ||
          x == 16777216 || x == 33554432 || x == 67108864 || x == 134217728 ||
          x == 268435456 || x == 536870912 || x == 1073741824);
}

void mica_init(struct mica_kv* kv, int instance_id, int node_id, int num_bkts,
               int log_cap) {
  int i, j;
  if(instance_id == 0)
  {
        for(i=0;i<MICA_MAX_KEYS;i++)
        {
                pthread_rwlock_init(&rwlock[i], NULL);
        }
        printf("initialize rwlock %d\n", MICA_MAX_KEYS);
        for(i=0;i<MICA_MAX_KEYS;i++)
        {
                queue_setmap[i] = MITSUME_NOT_IN_QUEUE;
        }
        for(i=0;i<MICA_BUCKET;i++)
        {
                pthread_rwlock_init(&queue_lock[i], NULL);
                MITSUME_QUEUE[i] = createQueue(MICA_CACHE_QUEUE_LENGTH/MICA_BUCKET);
        }
  }

  /* Verify struct sizes */
  assert(sizeof(struct mica_slot) == 8);
  assert(sizeof(struct mica_key) == 16);
  assert(sizeof(struct mica_op) % 64 == 0);

  assert(kv != NULL);
  assert(node_id == 0 || node_id == 1);

  /* 16 million buckets = a 1 GB index */
  assert(is_power_of_2(num_bkts) == 1 && num_bkts <= M_16);
  assert(log_cap > 0 && log_cap <= M_1024 && log_cap % M_2 == 0 &&
         is_power_of_2(log_cap));

  assert(MICA_LOG_BITS >= 24); /* Minimum log size = 16 MB */

  hrd_red_printf(
      "mica: Initializing MICA instance %d.\n"
      "NUMA node = %d, buckets = %d (size = %u B), log capacity = %d B.\n",
      instance_id, node_id, num_bkts, num_bkts * sizeof(struct mica_bkt),
      log_cap);

  if (MICA_DEBUG != 0) {
    printf("mica: Debug mode is ON! This might reduce performance.\n");
    sleep(2);
  }

  /* Initialize metadata and stats */
  kv->instance_id = instance_id;
  kv->node_id = node_id;

  kv->num_bkts = num_bkts;
  kv->bkt_mask = num_bkts - 1; /* num_bkts is power of 2 */

  kv->log_cap = log_cap;
  kv->log_mask = log_cap - 1; /* log_cap is a power of 2 */
  kv->log_head = 0;

  kv->num_get_op = 0;
  kv->num_get_fail = 0;
  kv->num_put_op = 0;
  kv->num_index_evictions = 0;

  /* Alloc index and initialize all entries to invalid */
  printf("mica: Allocting hash table index for instance %d\n", instance_id);
  int ht_index_key = MICA_INDEX_SHM_KEY + instance_id;
  kv->ht_index = (struct mica_bkt*)hrd_malloc_socket(
      ht_index_key, num_bkts * sizeof(struct mica_bkt), node_id);

  for (i = 0; i < num_bkts; i++) {
    for (j = 0; j < 8; j++) {
      kv->ht_index[i].slots[j].in_use = 0;
    }
  }

  /* Alloc log */
  printf("mica: Allocting hash table log for instance %d\n", instance_id);
  int ht_log_key = MICA_LOG_SHM_KEY + instance_id;
  kv->ht_log = (uint8_t*)hrd_malloc_socket(ht_log_key, log_cap, node_id);
}

void mica_insert_one(struct mica_kv* kv, struct mica_op* op,
                     struct mica_resp* resp) {
#if MICA_DEBUG == 1
  assert(kv != NULL);
  assert(op != NULL);
  assert(op->opcode == MICA_OP_PUT);
  assert(op->val_len > 0 && op->val_len <= MICA_MAX_VALUE);
  assert(resp != NULL);
#endif

  int i;
  unsigned int bkt = op->key.bkt & kv->bkt_mask;
  struct mica_bkt* bkt_ptr = &kv->ht_index[bkt];
  unsigned int tag = op->key.tag;

#if MICA_DEBUG == 2
  mica_print_op(op);
#endif

  kv->num_put_op++;

  /* Find a slot to use for this key. If there is a slot with the same
   * tag as ours, we are sure to find it because the used slots are at
   * the beginning of the 8-slot array. */
  int slot_to_use = -1;
  for (i = 0; i < 8; i++) {
    if (bkt_ptr->slots[i].tag == tag || bkt_ptr->slots[i].in_use == 0) {
      slot_to_use = i;
    }
  }

  /* If no slot found, choose one to evict */
  if (slot_to_use == -1) {
    slot_to_use = tag & 7; /* tag is ~ randomly distributed */
    kv->num_index_evictions++;
  }

  /* Encode the empty slot */
  bkt_ptr->slots[slot_to_use].in_use = 1;
  bkt_ptr->slots[slot_to_use].offset = kv->log_head; /* Virtual head */
  bkt_ptr->slots[slot_to_use].tag = tag;

  /* Paste the key-value into the log */
  uint8_t* log_ptr = &kv->ht_log[kv->log_head & kv->log_mask];

  /* Data copied: key, opcode, val_len, value */
  int len_to_copy =
      sizeof(struct mica_key) + sizeof(uint8_t) + sizeof(uint8_t) + op->val_len;

  /* Ensure that we don't wrap around in the *virtual* log space even
   * after 8-byte alignment below.*/
  assert((1ULL << MICA_LOG_BITS) - kv->log_head > len_to_copy + 8);

  memcpy(log_ptr, op, len_to_copy);
  kv->log_head += len_to_copy;

  /* Ensure that the key field of each log entry is 8-byte aligned. This
   * makes subsequent comparisons during GETs faster. */
  kv->log_head = (kv->log_head + 7) & ~7;

  /* If we're close to overflowing in the physical log, wrap around to
   * the beginning, but go forward in the virtual log. */
  if (unlikely(kv->log_cap - kv->log_head <= MICA_MAX_VALUE + 32)) {
    kv->log_head = (kv->log_head + kv->log_cap) & ~kv->log_mask;
    hrd_red_printf("mica: Instance %d wrapping around. Wraps = %llu\n",
                   kv->instance_id, kv->log_head / kv->log_cap);
  }
}

int mitsume_read_one_key_post(unsigned long long tar_key, int tar_mem, struct hrd_ctrl_blk** mem_cb,
                   void *mitsume_space, struct ibv_mr *mitsume_mr,
                   struct hrd_qp_attr ***mr_qp_list, int wrkr_lid, int verification, long long *traffic)
{
        struct ibv_sge test_sge;
        struct ibv_send_wr wr, *bad_send_wr;
        int ret;
        struct hrd_qp_attr **mr_qp;
        mr_qp = mr_qp_list[tar_mem];

        if (verification)
                test_sge.length = 8;
        else
                test_sge.length = MICA_HERD_VALUE_SIZE;

        test_sge.addr = (uintptr_t)mitsume_space;
        test_sge.lkey = mitsume_mr->lkey;

        wr.opcode = IBV_WR_RDMA_READ;
        wr.num_sge = 1;
        wr.next = NULL;
        wr.sg_list = &test_sge;
        wr.send_flags = IBV_SEND_SIGNALED;
        if(verification)
                wr.wr.rdma.remote_addr = mr_qp[tar_key]->buf_addr + MICA_HERD_VALUE_SIZE-8;
        else
                wr.wr.rdma.remote_addr = mr_qp[tar_key]->buf_addr;
        wr.wr.rdma.rkey = mr_qp[tar_key]->rkey;
        //printf("worker read %llu %llx %lx\n", tar_key, (long long unsigned int)wr.wr.rdma.remote_addr, (long unsigned int)wr.wr.rdma.rkey);
        ret = ibv_post_send(mem_cb[0]->conn_qp[tar_mem], &wr, &bad_send_wr);
        CPE(ret, "ibv_post_send error", ret);
        *traffic = *traffic + test_sge.length;
        return tar_mem;
}

int mitsume_write_one_key_post(unsigned long long tar_key, int tar_mem, struct hrd_ctrl_blk** mem_cb, 
                   void *mitsume_space, struct ibv_mr *mitsume_mr,
                   struct hrd_qp_attr ***mr_qp_list, int wrkr_lid, long long *traffic)
{
        struct ibv_sge test_sge;
        struct ibv_send_wr wr, *bad_send_wr;
        int ret;
        struct hrd_qp_attr **mr_qp;
        //unsigned long long tar_key = op[I]->key.bkt;
        mr_qp = mr_qp_list[tar_mem];

        test_sge.length = MICA_HERD_VALUE_SIZE;
        test_sge.addr = (uintptr_t)mitsume_space;
        test_sge.lkey = mitsume_mr->lkey;
        wr.opcode = IBV_WR_RDMA_WRITE;
        wr.num_sge = 1;
        wr.next = NULL;
        wr.sg_list = &test_sge;
        wr.send_flags = IBV_SEND_SIGNALED;
        wr.wr.rdma.remote_addr = mr_qp[tar_key]->buf_addr;
        wr.wr.rdma.rkey = mr_qp[tar_key]->rkey;
        ret = ibv_post_send(mem_cb[0]->conn_qp[tar_mem], &wr, &bad_send_wr);
        CPE(ret, "ibv_post_send error", ret);
        *traffic = *traffic + test_sge.length;
        return tar_mem;
}

void mitsume_poll(struct hrd_ctrl_blk** mem_cb, int tar_mem, int target_number)
{
        struct ibv_wc wc[MITSUME_MICA_WINDOW_SIZE];
        hrd_poll_cq(mem_cb[0]->conn_cq[tar_mem], 1, wc);
}

void mitsume_process(struct mica_op **op, int n, 
                        struct hrd_ctrl_blk** mem_cb,
                        void **mitsume_space_list, struct ibv_mr **mitsume_mr_list,
                        struct hrd_qp_attr ***mr_qp_list, struct hrd_qp_attr ***backupmr_qp_list,
                        int wrkr_lid, long long *send_traffic, long long *recv_traffic)
{
        
        int original_process[MICA_MAX_BATCH_SIZE]={0};
        const int NOT_YET_PROCESS = 0;
        const int GET_LOCK = 1;
        const int GENERATE_REQUEST = 2;
        const int FINISHED = 3;
        const int GENERATE_REQUEST_2 = 4;
        const int GENERATE_REQUEST_3 = 5;
        int finish_flag = 0;
        int ret;
        int tarmem_counter[MICA_NUM_MEMORY]={0};
        int tar_mem;
        int per_replication;
        unsigned long long tar_key;
        
        int i, j;
        
        if(!MICA_UNDOLOG_MODE)
        {
                for(i=0;i<n;i++)
                {
                        tar_key = op[i]->key.__unused;
                        if (op[i]->opcode == MICA_OP_PUT)
                        {
                                for(per_replication=0;per_replication<MICA_NUM_REPLICATION;per_replication++)
                                {
                                        tar_mem = (tar_key+per_replication) % MICA_NUM_MEMORY;
                                        ret = mitsume_write_one_key_post(tar_key, tar_mem, mem_cb, mitsume_space_list[i], mitsume_mr_list[i], backupmr_qp_list, wrkr_lid, send_traffic);
                                        tarmem_counter[ret]++;
                                }
                        }
                }
                //poll all write request
                //printf("%d: start polling %d\n", __LINE__, wrkr_lid);
                for(j=0;j<MICA_NUM_MEMORY;j++)
                {
                        if(tarmem_counter[j])
                                mitsume_poll(mem_cb, j, tarmem_counter[j]);
                }
                
                memset(tarmem_counter, 0, sizeof(int) * MICA_NUM_MEMORY);
                #ifdef MICA_READ_VERIFICATION
                for(i=0;i<n;i++)
                {
                        tar_key = op[i]->key.__unused;
                        if(op[i]->opcode == MICA_OP_PUT)
                        {
                                for(per_replication=0;per_replication<MICA_NUM_REPLICATION;per_replication++)
                                {
                                        tar_mem = (tar_key+per_replication) % MICA_NUM_MEMORY;
                                        ret = mitsume_read_one_key_post(tar_key, tar_mem, mem_cb, mitsume_space_list[i], mitsume_mr_list[i], backupmr_qp_list, wrkr_lid, 1, recv_traffic);
                                        tarmem_counter[ret]++;
                                }
                        }
                }
                //poll all validation write request
                for(j=0;j<MICA_NUM_MEMORY;j++)
                {
                        if(tarmem_counter[j])
                                mitsume_poll(mem_cb, j, tarmem_counter[j]);
                }
                memset(tarmem_counter, 0, sizeof(int) * MICA_NUM_MEMORY);
                #endif
        }
        while(!finish_flag)
        {
                memset(tarmem_counter, 0, sizeof(int) * MICA_NUM_MEMORY);

                //obtain lock
                for(i=0;i<n;i++)
                {
                        tar_key = op[i]->key.__unused;
                        if(original_process[i] == NOT_YET_PROCESS)
                        {
                                if (op[i]->opcode == MICA_OP_GET)
                                        ret = pthread_rwlock_tryrdlock(&rwlock[tar_key]);
                                else
                                        ret = pthread_rwlock_trywrlock(&rwlock[tar_key]);
                                if(ret == 0)
                                        original_process[i] = GET_LOCK;
                        }
                }

                //issue request for all read which already obtains lock
                for(i=0;i<n;i++)
                {
                        tar_key = op[i]->key.__unused;
                        tar_mem = tar_key % MICA_NUM_MEMORY;
                        if(original_process[i] == GET_LOCK && op[i]->opcode == MICA_OP_GET)
                        {
                                #ifdef MICA_USE_CACHE_QUEUE
                                int tar_bucket = tar_key%MICA_BUCKET;
                                pthread_rwlock_rdlock(&queue_lock[tar_bucket]);
                                if(queue_setmap[tar_key] == MITSUME_IN_QUEUE)
                                {
                                        pthread_rwlock_unlock(&queue_lock[tar_bucket]);
                                        original_process[i] = FINISHED;
                                        ret = pthread_rwlock_unlock(&rwlock[tar_key]);
                                        continue;
                                }
                                pthread_rwlock_unlock(&queue_lock[tar_bucket]);
                                #endif

                                ret = mitsume_read_one_key_post(tar_key, tar_mem, mem_cb, mitsume_space_list[i], mitsume_mr_list[i], mr_qp_list, wrkr_lid, 0, recv_traffic);
                                tarmem_counter[ret]++;
                                original_process[i] = GENERATE_REQUEST;
                        }
                }
                //poll all read request
                for(j=0;j<MICA_NUM_MEMORY;j++)
                {
                        if(tarmem_counter[j])
                                mitsume_poll(mem_cb, j, tarmem_counter[j]);
                }
                memset(tarmem_counter, 0, sizeof(int) * MICA_NUM_MEMORY);
                //release all read lock
                for(i=0;i<n;i++)
                {
                        tar_key = op[i]->key.__unused;
                        if(original_process[i] == GENERATE_REQUEST && op[i]->opcode == MICA_OP_GET)
                        {
                                original_process[i] = FINISHED;
                                ret = pthread_rwlock_unlock(&rwlock[tar_key]);
                                
                                #ifdef MICA_USE_CACHE_QUEUE
                                int tar_bucket = tar_key%MICA_BUCKET;
                                ret = pthread_rwlock_wrlock(&queue_lock[tar_bucket]);
                                if(queue_setmap[tar_key] == MITSUME_NOT_IN_QUEUE)
                                {
                                        if(isFull(MITSUME_QUEUE[tar_bucket]))
                                        {
                                                int tmp = dequeue(MITSUME_QUEUE[tar_bucket]);
                                                queue_setmap[tmp] = MITSUME_NOT_IN_QUEUE;
                                        }
                                        enqueue(MITSUME_QUEUE[tar_bucket], tar_key);
                                        queue_setmap[tar_key] = MITSUME_IN_QUEUE;
                                }
                                ret = pthread_rwlock_unlock(&queue_lock[tar_bucket]);
                                #endif
                        }
                }

                
                //start processing write request
                if(MICA_UNDOLOG_MODE)
                {

                        //step1 issue read request to get all data from mr
                        for(i=0;i<n;i++)
                        {
                                tar_key = op[i]->key.__unused;
                                tar_mem = tar_key % MICA_NUM_MEMORY;
                                if(original_process[i] == GET_LOCK && op[i]->opcode == MICA_OP_PUT)
                                {
                                        ret = mitsume_read_one_key_post(tar_key, tar_mem, mem_cb, mitsume_space_list[i], mitsume_mr_list[i], mr_qp_list, wrkr_lid, 0, recv_traffic);
                                        tarmem_counter[ret]++;
                                        original_process[i] = GENERATE_REQUEST;
                                }
                        }
                        //poll all read request
                        for(j=0;j<MICA_NUM_MEMORY;j++)
                        {
                                if(tarmem_counter[j])
                                        mitsume_poll(mem_cb, j, tarmem_counter[j]);
                        }
                        memset(tarmem_counter, 0, sizeof(int) * MICA_NUM_MEMORY);
                        //step2 issue write request to put all data to backupmr
                        for(i=0;i<n;i++)
                        {
                                tar_key = op[i]->key.__unused;
                                if(original_process[i] == GENERATE_REQUEST && op[i]->opcode == MICA_OP_PUT)
                                {
                                        for(per_replication=0;per_replication<MICA_NUM_REPLICATION;per_replication++)
                                        {
                                                tar_mem = (tar_key+per_replication) % MICA_NUM_MEMORY;
                                                ret = mitsume_write_one_key_post(tar_key, tar_mem, mem_cb, mitsume_space_list[i], mitsume_mr_list[i], backupmr_qp_list, wrkr_lid, send_traffic);
                                                tarmem_counter[ret]++;
                                        }
                                        original_process[i] = GENERATE_REQUEST_2;
                                }
                        }
                        //poll all write request
                        for(j=0;j<MICA_NUM_MEMORY;j++)
                        {
                                if(tarmem_counter[j])
                                        mitsume_poll(mem_cb, j, tarmem_counter[j]);
                        }
                        memset(tarmem_counter, 0, sizeof(int) * MICA_NUM_MEMORY);
                        #ifdef MICA_READ_VERIFICATION
                        //step 2.1
                        //issue a read request to flush all write data
                        for(i=0;i<n;i++)
                        {
                                tar_key = op[i]->key.__unused;
                                if(original_process[i] == GENERATE_REQUEST_2 && op[i]->opcode == MICA_OP_PUT)
                                {
                                        for(per_replication=0;per_replication<MICA_NUM_REPLICATION;per_replication++)
                                        {
                                                tar_mem = (tar_key+per_replication) % MICA_NUM_MEMORY;
                                                ret = mitsume_read_one_key_post(tar_key, tar_mem, mem_cb, mitsume_space_list[i], mitsume_mr_list[i], backupmr_qp_list, wrkr_lid, 1, recv_traffic);
                                                tarmem_counter[ret]++;
                                        }
                                }
                        }
                        //poll all validation write request
                        for(j=0;j<MICA_NUM_MEMORY;j++)
                        {
                                if(tarmem_counter[j])
                                        mitsume_poll(mem_cb, j, tarmem_counter[j]);
                        }
                        memset(tarmem_counter, 0, sizeof(int) * MICA_NUM_MEMORY);
                        #endif

                        //step3 issue write request to write data to mr
                        for(i=0;i<n;i++)
                        {
                                tar_key = op[i]->key.__unused;
                                if(original_process[i] == GENERATE_REQUEST_2 && op[i]->opcode == MICA_OP_PUT)
                                {
                                        for(per_replication=0;per_replication<MICA_NUM_REPLICATION;per_replication++)
                                        {
                                                tar_mem = (tar_key+per_replication) % MICA_NUM_MEMORY;
                                                ret = mitsume_write_one_key_post(tar_key, tar_mem, mem_cb, mitsume_space_list[i], mitsume_mr_list[i], mr_qp_list, wrkr_lid, send_traffic);
                                                tarmem_counter[ret]++;
                                        }
                                        original_process[i] = GENERATE_REQUEST_3;
                                }
                        }
                        //poll all write request
                        for(j=0;j<MICA_NUM_MEMORY;j++)
                        {
                                if(tarmem_counter[j])
                                        mitsume_poll(mem_cb, j, tarmem_counter[j]);
                        }
                        memset(tarmem_counter, 0, sizeof(int) * MICA_NUM_MEMORY);
                        
                        #ifdef MICA_READ_VERIFICATION
                        //step 3.1
                        //issue a read request to flush all write data
                        for(i=0;i<n;i++)
                        {
                                tar_key = op[i]->key.__unused;
                                if(original_process[i] == GENERATE_REQUEST_3 && op[i]->opcode == MICA_OP_PUT)
                                {
                                        for(per_replication=0;per_replication<MICA_NUM_REPLICATION;per_replication++)
                                        {
                                                tar_mem = (tar_key+per_replication) % MICA_NUM_MEMORY;
                                                ret = mitsume_read_one_key_post(tar_key, tar_mem, mem_cb, mitsume_space_list[i], mitsume_mr_list[i], mr_qp_list, wrkr_lid, 1, recv_traffic);
                                                tarmem_counter[ret]++;
                                        }
                                }
                        }
                        //poll all validation write request
                        for(j=0;j<MICA_NUM_MEMORY;j++)
                        {
                                if(tarmem_counter[j])
                                        mitsume_poll(mem_cb, j, tarmem_counter[j]);
                        }
                        memset(tarmem_counter, 0, sizeof(int) * MICA_NUM_MEMORY);
                        #endif

                        //release all write lock
                        for(i=0;i<n;i++)
                        {
                                tar_key = op[i]->key.__unused;
                                if(original_process[i] == GENERATE_REQUEST_3 && op[i]->opcode == MICA_OP_PUT)
                                {
                                        original_process[i] = FINISHED;
                                        ret = pthread_rwlock_unlock(&rwlock[tar_key]);
                                        
                                        #ifdef MICA_USE_CACHE_QUEUE
                                        int tar_bucket = tar_key%MICA_BUCKET;
                                        ret = pthread_rwlock_wrlock(&queue_lock[tar_bucket]);
                                        if(queue_setmap[tar_key] == MITSUME_NOT_IN_QUEUE)
                                        {
                                                if(isFull(MITSUME_QUEUE[tar_bucket]))
                                                {
                                                        int tmp = dequeue(MITSUME_QUEUE[tar_bucket]);
                                                        queue_setmap[tmp] = MITSUME_NOT_IN_QUEUE;
                                                }
                                                enqueue(MITSUME_QUEUE[tar_bucket], tar_key);
                                                queue_setmap[tar_key] = MITSUME_IN_QUEUE;
                                        }
                                        ret = pthread_rwlock_unlock(&queue_lock[tar_bucket]);
                                        #endif
                                }
                        }
                }
                else//redo log
                {
                        /*
                        for(i=0;i<n;i++)
                        {
                                tar_key = op[i]->key.__unused;
                                if (original_process[i] == GET_LOCK && op[i]->opcode == MICA_OP_PUT)
                                {
                                        for(per_replication=0;per_replication<MICA_NUM_REPLICATION;per_replication++)
                                        {
                                                tar_mem = (tar_key+per_replication) % MICA_NUM_MEMORY;
                                                ret = mitsume_write_one_key_post(tar_key, tar_mem, mem_cb, mitsume_space_list[i], mitsume_mr_list[i], backupmr_qp_list, wrkr_lid);
                                                tarmem_counter[ret]++;
                                        }
                                        original_process[i] = GENERATE_REQUEST;
                                }
                        }
                        //poll all write request
                        //printf("%d: start polling %d\n", __LINE__, wrkr_lid);
                        for(j=0;j<MICA_NUM_MEMORY;j++)
                        {
                                if(tarmem_counter[j])
                                        mitsume_poll(mem_cb, j, tarmem_counter[j]);
                        }
                        
                        memset(tarmem_counter, 0, sizeof(int) * MICA_NUM_MEMORY);
                        #ifdef MICA_READ_VERIFICATION
                        for(i=0;i<n;i++)
                        {
                                tar_key = op[i]->key.__unused;
                                if(original_process[i] == GENERATE_REQUEST && op[i]->opcode == MICA_OP_PUT)
                                {
                                        for(per_replication=0;per_replication<MICA_NUM_REPLICATION;per_replication++)
                                        {
                                                tar_mem = (tar_key+per_replication) % MICA_NUM_MEMORY;
                                                ret = mitsume_read_one_key_post(tar_key, tar_mem, mem_cb, mitsume_space_list[i], mitsume_mr_list[i], backupmr_qp_list, wrkr_lid, 1);
                                                tarmem_counter[ret]++;
                                        }
                                }
                        }
                        //poll all validation write request
                        for(j=0;j<MICA_NUM_MEMORY;j++)
                        {
                                if(tarmem_counter[j])
                                        mitsume_poll(mem_cb, j, tarmem_counter[j]);
                        }
                        memset(tarmem_counter, 0, sizeof(int) * MICA_NUM_MEMORY);
                        #endif
                        */
                        //switch the metadata between backupmr and majormr
                        
                        
                        for(i=0;i<n;i++)
                        {
                                if (original_process[i] == GET_LOCK && op[i]->opcode == MICA_OP_PUT)
                                {
                                        original_process[i] = GENERATE_REQUEST;
                                }
                        }
                        
                        /*for(i=0;i<n;i++)
                        {
                                struct hrd_qp_attr swap;
                                struct hrd_qp_attr **mr_qp, **backup_mr_qp;
                                tar_key = op[i]->key.__unused;
                                if(original_process[i] == GENERATE_REQUEST && op[i]->opcode == MICA_OP_PUT)
                                //dont really switch to avoid design issue
                                {
                                        for(per_replication=0;per_replication<MICA_NUM_REPLICATION;per_replication++)
                                        {
                                                tar_mem = (tar_key+per_replication) % MICA_NUM_MEMORY;
                                                backup_mr_qp = backupmr_qp_list[tar_mem];
                                                mr_qp = mr_qp_list[tar_mem];
                                                
                                                swap.buf_addr = mr_qp[tar_key]->buf_addr;
                                                swap.rkey = mr_qp[tar_key]->rkey;
                                                mr_qp[tar_key]->buf_addr = backup_mr_qp[tar_key]->buf_addr;
                                                mr_qp[tar_key]->rkey = backup_mr_qp[tar_key]->rkey;
                                                backup_mr_qp[tar_key]->buf_addr = swap.buf_addr;
                                                backup_mr_qp[tar_key]->rkey = swap.rkey;
                                        }
                                }
                        }*/
                        

                        //release all write lock
                        for(i=0;i<n;i++)
                        {
                                tar_key = op[i]->key.__unused;
                                if(original_process[i] == GENERATE_REQUEST && op[i]->opcode == MICA_OP_PUT)
                                {
                                        original_process[i] = FINISHED;
                                        ret = pthread_rwlock_unlock(&rwlock[tar_key]);
                                        
                                        #ifdef MICA_USE_CACHE_QUEUE
                                        int tar_bucket = tar_key%MICA_BUCKET;
                                        ret = pthread_rwlock_wrlock(&queue_lock[tar_bucket]);
                                        if(queue_setmap[tar_key] == MITSUME_NOT_IN_QUEUE)
                                        {
                                                if(isFull(MITSUME_QUEUE[tar_bucket]))
                                                {
                                                        int tmp = dequeue(MITSUME_QUEUE[tar_bucket]);
                                                        queue_setmap[tmp] = MITSUME_NOT_IN_QUEUE;
                                                }
                                                enqueue(MITSUME_QUEUE[tar_bucket], tar_key);
                                                queue_setmap[tar_key] = MITSUME_IN_QUEUE;
                                        }
                                        ret = pthread_rwlock_unlock(&queue_lock[tar_bucket]);
                                        #endif
                                }
                        }
                }
                //verify finish status
                finish_flag = true;
                for(i=0;i<n;i++)
                {
                        if(original_process[i]!=FINISHED)
                        {
                                finish_flag = false;
                                break;
                        }
                }
        }
}

/*
 * Process a batch of operations.
 * The caller expects @mica_resp to have valid @type, @val_len, and @val_ptr
 * fields. For failed GETs and all PUTs, @val_len should be 0 so that HERD can
 * send a 0-byte response. In these cases, the value of @val_ptr doesn't matter.
 */
void mica_batch_op(struct mica_kv* kv, int n, struct mica_op** op,
                   struct mica_resp* resp, struct hrd_ctrl_blk** mem_cb,
                   void **mitsume_space_list, struct ibv_mr **mitsume_mr_list,
                   struct hrd_qp_attr ***mr_qp_list, struct hrd_qp_attr ***backupmr_qp_list,
                   int wrkr_lid, long long *send_traffic, long long *recv_traffic) {
  int I, j; /* I is batch index */
#if MICA_DEBUG == 1
  assert(kv != NULL);
  assert(op != NULL);
  assert(n > 0 && n <= MICA_MAX_BATCH_SIZE);
  assert(resp != NULL);
#endif

#if MICA_DEBUG == 2
  for (I = 0; I < n; I++) {
    mica_print_op(op[I]);
  }
#endif

  unsigned int bkt[MICA_MAX_BATCH_SIZE];
  struct mica_bkt* bkt_ptr[MICA_MAX_BATCH_SIZE];
  unsigned int tag[MICA_MAX_BATCH_SIZE];
  int key_in_store[MICA_MAX_BATCH_SIZE]; /* Is this key in the datastore? */
  struct mica_op* kv_ptr[MICA_MAX_BATCH_SIZE]; /* Ptr to KV item in log */

  /*
   * We first lookup the key in the datastore. The first two @I loops work
   * for both GETs and PUTs.
   */
  for (I = 0; I < n; I++) {
    bkt[I] = op[I]->key.bkt & kv->bkt_mask;
    bkt_ptr[I] = &kv->ht_index[bkt[I]];
    __builtin_prefetch(bkt_ptr[I], 0, 0);
    tag[I] = op[I]->key.tag;

    key_in_store[I] = 0;
    kv_ptr[I] = NULL;
  }

  for (I = 0; I < n; I++) {
    for (j = 0; j < 8; j++) {
      if (bkt_ptr[I]->slots[j].in_use == 1 &&
          bkt_ptr[I]->slots[j].tag == tag[I]) {
        uint64_t log_offset = bkt_ptr[I]->slots[j].offset & kv->log_mask;

        /*
         * We can interpret the log entry as mica_op, even though it
         * may not contain the full MICA_MAX_VALUE value.
         */
        kv_ptr[I] = (struct mica_op*)&kv->ht_log[log_offset];

        /* Small values (1--64 bytes) can span 2 cache lines */
        __builtin_prefetch(kv_ptr[I], 0, 0);
        __builtin_prefetch((uint8_t*)kv_ptr[I] + 64, 0, 0);

        /* Detect if the head has wrapped around for this index entry */
        if (kv->log_head - bkt_ptr[I]->slots[j].offset >= kv->log_cap) {
          kv_ptr[I] = NULL; /* If so, we mark it "not found" */
        }

        break;
      }
    }
  }
  for (I = 0; I < n; I++) {
        if (op[I]->opcode == MICA_OP_GET)
                *recv_traffic = *recv_traffic + 16 + 1;//HERD_GET_REQ_SIZE
        else
                *recv_traffic = *recv_traffic + 16 + 1 + 1 + MICA_HERD_VALUE_SIZE;//HERD_PUT_REQ_SIZE
  }
  mitsume_process(op, n, mem_cb, mitsume_space_list, mitsume_mr_list, mr_qp_list, backupmr_qp_list, wrkr_lid, send_traffic, recv_traffic);
  for (I = 0; I < n; I++) {
    if (kv_ptr[I] != NULL) {
      /* We had a tag match earlier. Now compare log entry. */
      long long* key_ptr_log = (long long*)kv_ptr[I];
      long long* key_ptr_req = (long long*)op[I];

      if (key_ptr_log[0] == key_ptr_req[0] &&
          key_ptr_log[1] == key_ptr_req[1]) {
        key_in_store[I] = 1;

        if (op[I]->opcode == MICA_OP_GET) {
          kv->num_get_op++;

          resp[I].type = MICA_RESP_GET_SUCCESS;
          //resp[I].val_len = kv_ptr[I]->val_len;
          resp[I].val_len = MICA_HERD_VALUE_SIZE;
          resp[I].val_ptr = kv_ptr[I]->value;
          
        } else {
          kv->num_put_op++;
          /* Update the value in the log */
          op[I]->val_len = MICA_HERD_VALUE_SIZE;
          kv_ptr[I]->val_len = MICA_HERD_VALUE_SIZE;
          assert(op[I]->val_len == kv_ptr[I]->val_len);
          memcpy(kv_ptr[I]->value, op[I]->value, kv_ptr[I]->val_len);
          
          resp[I].type = MICA_RESP_PUT_SUCCESS;
          resp[I].val_len = 0;
          resp[I].val_ptr = NULL;
        }
      }
    }

    if (key_in_store[I] == 0) {
      /* We get here if either tag or log key match failed */
      if (op[I]->opcode == MICA_OP_GET) {
        kv->num_get_fail++;

        resp[I].type = MICA_RESP_GET_FAIL;
        resp[I].val_len = 0;
        resp[I].val_ptr = NULL;
      } else {
        /*
         * If datastore lookup failed for PUT, it's an INSERT. INSERTs
         * currently happen on a slow non-batched path.
         */
        mica_insert_one(kv, op[I], &resp[I]);

        resp[I].type = MICA_RESP_PUT_SUCCESS;
        resp[I].val_len = 0;
        resp[I].val_ptr = NULL;
      }
    }
  }
}

void mica_print_bucket(struct mica_kv* kv, int bkt_idx) {
  assert(kv != NULL);
  assert(bkt_idx > 0 && bkt_idx < kv->num_bkts);

  int i;
  struct mica_bkt* bkt = &kv->ht_index[bkt_idx];
  printf("mica: Bucket %d:\n", bkt_idx);
  for (i = 0; i < 8; i++) {
    printf("\tSlot %d: in_use = %u, offset = {%lu, %u} tag = %u\n", i,
           bkt->slots[i].in_use, (uint64_t)bkt->slots[i].offset,
           (uint32_t)(bkt->slots[i].offset & kv->log_mask), bkt->slots[i].tag);
  }
}

void mica_print_op(struct mica_op* op) {
  assert(op != NULL);

  int i;
  long long* key = (long long*)&op->key;

  if (op->opcode == MICA_OP_GET) {
    printf("mica: GET for key %llx %llx\n", key[0], key[1]);
  }

  if (op->opcode == MICA_OP_PUT) {
    printf("mica: PUT for key %llx %llx. Value = ", key[0], key[1]);
    assert(op->val_len > 0 && op->val_len <= MICA_MAX_VALUE);
    for (i = 0; i < op->val_len; i++) {
      printf("%d ", op->value[i]);
    }
    printf("\n");
  }
}

/* A fast deterministic way to generate @n ~randomly distributed 16-byte keys */
uint128* mica_gen_keys(int n) {
  int i;
  assert(n > 0 && n <= M_1024 / sizeof(uint128));
  assert(sizeof(uint128) == 16);

  printf("mica: Generating %d keys\n", n);

  uint128* key_arr = malloc(n * sizeof(uint128));
  assert(key_arr != NULL);

  for (i = 0; i < n; i++) {
    key_arr[i] = CityHash128((char*)&i, 4);
  }

  return key_arr;
}

/*
 * Populate the KVS with @n keys, each with value length = @val_len.
 *  - The ith key is the CityHash128 value of i.
 *  - The bytes of the value inserted are all equal to the least significant
 *    byte of the key.
 */
void mica_populate_fixed_len(struct mica_kv* kv, int n, int val_len) {
  assert(kv != NULL);
  assert(n > 0);
  assert(val_len > 0 && val_len <= MICA_MAX_VALUE);

  /* This is needed for the eviction message below to make sense */
  assert(kv->num_put_op == 0 && kv->num_index_evictions == 0);

  int i;
  struct mica_op op;
  unsigned long long* op_key = (unsigned long long*)&op.key;
  struct mica_resp resp;

  /* Generate the keys to insert */
  uint128* key_arr = mica_gen_keys(n);

  for (i = 0; i < n; i++) {
    op_key[0] = key_arr[i].first;
    op_key[1] = key_arr[i].second;
    op.opcode = MICA_OP_PUT;

    op.val_len = val_len;
    uint8_t val = (uint8_t)(op_key[1] & 0xff);
    memset(op.value, val, val_len);

    mica_insert_one(kv, &op, &resp);
  }

  assert(kv->num_put_op == n);
  printf(
      "mica: Populated instance %d with %d keys, length = %d. "
      "Index eviction fraction = %.4f.\n",
      kv->instance_id, n, val_len,
      (double)kv->num_index_evictions / kv->num_put_op);
}
