...
const unsigned long long int MIN_LH_IN_ONE_NODE = (unsigned long long)MITSUME_MEMORY_PER_ALLOCATION_KB*1024/MITSUME_MEMORY_LH_CUT_MAX_UNIT;
 ...
 */
uint32_t mitsume_con_alloc_rr_get_controller_id(struct mitsume_consumer_metadata *thread_metadata)
{
...
uint32_t mitsume_con_alloc_key_to_controller_id(mitsume_key key)
{
        ...
        //[TODO] this should map to a controller based on static hash (same for all clients)
}


/**
 * mitsume_con_alloc_size_to_alloc_num - map size into correct list (one lh always maps to same size)
 * @size: request size
...
/**
 * mitsume_con_alloc_pointer_to_size - map pointer to correct size based on replication_factor (-sizeof pointer*replication_factor)
 * @lh: lh
 * @replication_factor: 
 * return: return correct size
 */
uint32_t mitsume_con_alloc_pointer_to_size(uint64_t pointer, uint32_t replication_factor)
{
        return mitsume_con_alloc_lh_to_size(MITSUME_GET_PTR_LH(pointer)) - sizeof(struct mitsume_ptr) * replication_factor - MITSUME_CRC_SIZE;
}
...
int mitsume_con_alloc_share_init(void)
{
    ...
    {
        MITSUME_CON_ALLOC_BLOCK[i] = MITSUME_CON_ALLOCATOR_SLAB_SIZE_GRANULARITY << i;
        MITSUME_PRINT("list:size (%d:%d)\n", i, (int)MITSUME_CON_ALLOC_BLOCK[i]);
        if(MITSUME_CON_ALLOC_BLOCK[i]>MITSUME_MEMORY_LH_CUT_MAX_UNIT ||  MITSUME_MEMORY_LH_CUT_MAX_UNIT%MITSUME_CON_ALLOC_BLOCK[i]!=0)
        {   
            die_printf("error BLOCK size (%d:%d)", MITSUME_CON_ALLOC_BLOCK[i], MITSUME_MEMORY_LH_CUT_MAX_UNIT);
        }
    }
    ...
    {
        cur_offset = 0;
        tar_list = mitsume_con_alloc_internal_lh_to_list_num(cur_lh_num);//get size from set_lh_num
        cur_size = mitsume_con_alloc_list_num_to_size(tar_list);//get size from set_lh_num
    ...
    {
        //MITSUME_PRINT("%d, %d, %llu\n", per_slab, mitsume_con_alloc_list_num_to_size(per_slab), list_count[per_slab]);
...
int mitsume_con_alloc_populate_lh(struct mitsume_ctx_con *local_ctx_con)
{
    ...
    for(i=0;i<MITSUME_MEM_NUM;i++)
    {
        ...
        memcached_publish(memcached_string, &local_ctx_con->all_lh_attr[MITSUME_PER_NODE_LH*i], sizeof(ptr_attr)*MITSUME_PER_NODE_LH);
    }

    //memcached_publish(memcached_string, local_ctx_con->all_lh_attr, sizeof(ptr_attr)*mitsume_con_alloc_get_total_lh());
    return MITSUME_SUCCESS;
}

int mitsume_con_alloc_get_lh(struct mitsume_ctx_con *local_ctx_con, struct mitsume_ctx_clt *local_ctx_clt)
{
    ...
    tmp_attr = memcached_get_published_size(memcached_string, sizeof(ptr_attr)*mitsume_con_alloc_get_total_lh());
    memcpy(local_ctx_con->all_lh_attr, tmp_attr, sizeof(ptr_attr)*mitsume_con_alloc_get_total_lh());
    free(tmp_attr);*/
    
    for(i=0;i<MITSUME_MEM_NUM;i++)
    {
        sprintf(memcached_string, MITSUME_MEMCACHED_LH_STRING, i);
        tmp_attr = memcached_get_published_size(memcached_string, sizeof(ptr_attr)*MITSUME_PER_NODE_LH);
        if(local_ctx_con)
            memcpy(&local_ctx_con->all_lh_attr[i*MITSUME_PER_NODE_LH], tmp_attr, sizeof(ptr_attr)*MITSUME_PER_NODE_LH);
        else
            memcpy(&local_ctx_clt->all_lh_attr[i*MITSUME_PER_NODE_LH], tmp_attr, sizeof(ptr_attr)*MITSUME_PER_NODE_LH);
        free(tmp_attr);
    }
...
/**
 * mitsume_con_alloc_split_lh_into_entries - split and divide all available LH into entries
 * @local_ctx_con: controller context
 * return: return 0 for success, -1 for error happens
 */
int mitsume_con_alloc_split_space_into_lh(struct mitsume_ctx_con *local_ctx_con)
{
    int i;
    ...
    unsigned long long MIN_LH_IN_ONE_NODE = (unsigned long long)MITSUME_MEMORY_PER_ALLOCATION_KB*1024/MITSUME_MEMORY_LH_CUT_MAX_UNIT;
    unsigned long long MAX_LH_IN_ONE_NODE = (unsigned long long)MITSUME_MEMORY_PER_ALLOCATION_KB*1024/MITSUME_MEMORY_LH_CUT_MIN_UNIT;
    int tar_list;
    ...
    for(i=0;i<MITSUME_MEM_NUM;i++)//first cut memory space into lh
    {
        ...
        for(cur_lh_num=0;cur_lh_num<MIN_LH_IN_ONE_NODE;cur_lh_num++)
        {
            ...
            while(cur_offset<MITSUME_MEMORY_LH_CUT_MAX_UNIT)
            {
                memory_cut[total_mem_count].addr = local_ctx_con->all_memory_attr[i].addr+cur_base+cur_offset;
                memory_cut[total_mem_count].rkey = local_ctx_con->all_memory_attr[i].rkey;
                memory_cut[total_mem_count].machine_id = local_ctx_con->all_memory_attr[i].machine_id;
                cur_offset += cur_size;
                real_lh_num++;
                list_count[tar_list]++;
                per_mem_count++;
...
/**
 * mitsume_con_alloc_pickthread - tell requester or allocater which place should be used for the entry
 * @local_ctx_con: controller context
 * @input_entry: target entry
 * return: return target thread
 */
int mitsume_con_alloc_pickthread_to_put(struct mitsume_ctx_con *local_ctx_con, struct mitsume_allocator_entry *input_entry)
{
    //return 0;//this place should be RR or other algorithms in the future
    return (MITSUME_GET_PTR_LH(input_entry->ptr.pointer))%MITSUME_CON_ALLOCATOR_THREAD_NUMBER;
}

/**
 * mitsume_con_alloc_lh_to_bucket_based_on_predefined_map - map lh to correct node id which is used for replication later
 * this mapping is shared from controller
 * @lh: lh
 ...
 */
int mitsume_con_alloc_put_entry_into_thread(struct mitsume_ctx_con *local_ctx_con, struct mitsume_allocator_entry *input_entry, int tail)
{
    int target_thread = mitsume_con_alloc_pickthread_to_put(local_ctx_con, input_entry);
    int target_list = mitsume_con_alloc_lh_to_list_num(MITSUME_GET_PTR_LH(input_entry->ptr.pointer));
    int target_replication_bucket = mitsume_con_alloc_lh_to_node_id_bucket(MITSUME_GET_PTR_LH(input_entry->ptr.pointer));
    //struct mitsume_allocator_entry *target_allocator_list = local_ctx_con->thread_metadata[target_thread].allocator_node_branch[target_replication_bucket];
    local_ctx_con->thread_metadata[target_thread].allocator_lock_branch[target_replication_bucket][target_list].lock();
    if(tail)
        local_ctx_con->thread_metadata[target_thread].allocator_node_branch[target_replication_bucket][target_list].push_back(input_entry->ptr.pointer);
    else
        local_ctx_con->thread_metadata[target_thread].allocator_node_branch[target_replication_bucket][target_list].push_front(input_entry->ptr.pointer);

    local_ctx_con->thread_metadata[target_thread].allocator_lock_branch[target_replication_bucket][target_list].unlock();
    return MITSUME_SUCCESS;
}


int mitsume_con_alloc_split_lh_into_entries(struct mitsume_ctx_con *local_ctx_con)
{
    ...
    for(cur_lh_num=0;cur_lh_num<MITSUME_PER_NODE_LH*MITSUME_MEM_NUM;cur_lh_num++)
    {
        ...
        tmp_entry.ptr.pointer = mitsume_struct_set_pointer(cur_lh_num, 0, MITSUME_ENTRY_MIN_VERSION, 0, 0);
        if(mitsume_con_alloc_put_entry_into_thread(local_ctx_con, &tmp_entry, MITSUME_CON_ALLOCATOR_PUT_TAIL))
    ...
    for(i=0;i<MITSUME_CON_ALLOCATOR_SLAB_NUMBER;i++)
    {
        ...
        for(per_bucket=0;per_bucket<MITSUME_NUM_REPLICATION_BUCKET;per_bucket++)
        {
            ...
            char local_use[128];
            for(per_thread=0;per_thread<MITSUME_CON_ALLOCATOR_THREAD_NUMBER;per_thread++)
                local_sum += local_ctx_con->thread_metadata[per_thread].allocator_node_branch[per_bucket][i].size();
        ...
        MITSUME_INFO("size:number (%llu:%llu) %s\n", (unsigned long long int)mitsume_con_alloc_list_num_to_size(i), record[i], count_string_array);
    }
    MITSUME_INFO("hold with %llu:%llu lh\n", lh_count, MITSUME_PER_NODE_LH*MITSUME_MEM_NUM);
    return MITSUME_SUCCESS;
}

/**
 * mitsume_con_alloc_put_shortcut_into_list - put available shortcut entry into the list which will be used in the future during receiving OPEN request
 * @local_ctx_con: local controller context
 * return: return success
 */
int mitsume_con_alloc_put_shortcut_into_list(struct mitsume_ctx_con *local_ctx_con)
{
    unsigned long long int per_sh;
    unsigned long long int available_shortcut=0;
    struct mitsume_shortcut_entry tmp_entry;
    for(per_sh=MITSUME_SHORTCUT_LH_BASE;per_sh<MITSUME_SHORTCUT_NUM;per_sh++)
    {
 ...
 */
int mitsume_con_alloc_get_shortcut_from_list(struct mitsume_allocator_metadata *thread_metadata, struct mitsume_shortcut_entry *output)
{
 ...
 * this function should be called no matter this node is a client or a controller
 * after calling this function, consumer can start getting available entries from controller
 * return: return 0 success, -1 error
 */
int mitsume_con_alloc_entry_init(struct configuration_params *input_arg, struct mitsume_ctx_con *server_ctx)
{
