...
std::unordered_map<uint64_t, struct mitsume_hash_struct*> MITSUME_CON_NAMER_HASHTABLE;

mutex MITSUME_CON_GC_HASHTABLE_CURRENT_LOCK[1<<MITSUME_CON_NAMER_HASHTABLE_SIZE_BIT];
std::unordered_map<uint64_t, struct mitsume_gc_hashed_entry*> MITSUME_CON_GC_HASHTABLE_CURRENT;

mutex MITSUME_CON_GC_HASHTABLE_BUFFER_LOCK[1<<MITSUME_CON_NAMER_HASHTABLE_SIZE_BIT];
std::unordered_map<uint64_t, struct mitsume_gc_hashed_entry*> MITSUME_CON_GC_HASHTABLE_BUFFER;*/
...
int mitsume_test_thread(int thread_num, struct mitsume_ctx_clt *local_ctx_clt, void* (*fun_ptr)(void *input_metadata))
{
    ...
    {
        MITSUME_INFO("read bucket-%d %ld\n", i, atomic64_read(&local_ctx_clt->read_bucket_counter[i]));
    }
    for(i=0;i<MITSUME_NUM_REPLICATION_BUCKET;i++)
    {
        MITSUME_INFO("write bucket-%d %ld\n", i, atomic64_read(&local_ctx_clt->write_bucket_counter[i]));
    }*/
    ...
    if(thread_metadata->thread_id == 0)
    {
        ...
        if(client_id == 0)
        {
            for(i=1;i<=key_range;i++)
            {
                ...
                recv = mitsume_clt_consumer_ask_entries_from_controller(thread_metadata, target_list, 16, 0, &ret_bucket);
                MITSUME_PRINT("%d-%d\n", i, recv);
                UNUSED(key);
                UNUSED(write);
                UNUSED(ret);*/
                ...
                ret = mitsume_tool_open(thread_metadata, key, write, test_size, MITSUME_TEST_ONE);
                if(ret)
            ...
            {
                MITSUME_PRINT("%llx\n", (unsigned long long int)thread_metadata->consumer_node_branch[0][0].front());
    ...
    if(thread_metadata->thread_id == 0)
    {
        ...
        if(client_id == 0)
        {
            MITSUME_PRINT("%llx:%llx:%llx\n", 
                (unsigned long long int)&thread_metadata->local_inf->chasing_shortcut[0], 
                (unsigned long long int)thread_metadata->local_inf->chasing_shortcut_mr[0].addr, 
                (unsigned long long int)thread_metadata->local_inf->chasing_shortcut_mr[0].lkey);
            for(i=1;i<=key_range;i++)
            {
                /*int recv;
                int ret_bucket;
...
/**
 * mitsume_test_correctness: test the correctness of basic keyvalue read write operation
 */
void *mitsume_test_correctness(void *input_metadata)
{
    ...
    if(thread_metadata->thread_id == 0)
    {
        ...
        {
            MITSUME_PRINT("------\n");
            //ret = mitsume_tool_open(thread_metadata, key, write, 36, MITSUME_TEST_ONE);
            ret = mitsume_tool_open(thread_metadata, key, write, 120, MITSUME_TEST_THREE);
            if(ret)
                MITSUME_INFO("fail to open %llu\n", (unsigned long long int) key);
            else
            ...
            for(i=0;i<test_times;i++)
            {
                ...
                mitsume_tool_write(thread_metadata, key, write, 104, MITSUME_TOOL_KVSTORE_WRITE);
                //mitsume_tool_write(thread_metadata, key, write, 36, MITSUME_TOOL_KVSTORE_WRITE);
                //mitsume_tool_write(thread_metadata, key, write, 36, MITSUME_TOOL_KVSTORE_WRITE);
                //MITSUME_PRINT("after write\n");
                //memset(read, 0, 120);
                mitsume_tool_read(thread_metadata, key, read, &read_size, MITSUME_TOOL_KVSTORE_READ);
                MITSUME_PRINT("%d:%c %u %.*s\n", i, write[0], read_size, read_size, (char *)read);
            }
...
void *mitsume_test_distribution(void *input_metadata)
{
    ...
    mitsume_key key;
    int test_size = MITSUME_TEST_SIZE - sizeof(struct mitsume_ptr) * MITSUME_TEST_THREE;
    ...
    write = (char *)local_inf->user_input_space[0];
    read = local_inf->user_output_space[0];
    target_write_set = thread_metadata->thread_id + (client_id)*MITSUME_CLT_TEST_THREAD_NUM;
    target_read_set = thread_metadata->thread_id + (client_id)*MITSUME_CLT_TEST_THREAD_NUM;
    ...
    {
        sprintf(write_string, MITSUME_TEST_WORKLOAD_UNIFORM_STRING, target_write_set);
    ...
    if(thread_metadata->thread_id == 0)
    {
        if(client_id==0)
        {
            ...
            {
                key = i;
                ret = mitsume_tool_open(thread_metadata, key, write, 36, MITSUME_TEST_THREE);
                if(ret)
                    MITSUME_INFO("fail to open %llu\n", (unsigned long long int) key);
                //else
                //    MITSUME_INFO("open %llu success\n", (unsigned long long int) key);
...
/**
 * mitsume_test_distribution: generate a bunch of thread to test concurrent kvstore read/write (most frequently used test)
 */
/*void *mitsume_test_distribution(void *input_metadata)
{
    struct mitsume_consumer_metadata *thread_metadata = (struct mitsume_consumer_metadata *)input_metadata;
    ...
    int test_times = 100;
    int key_range = MITSUME_TEST_RAND_KEY_RANGE;
    int test_size = MITSUME_TEST_SIZE - sizeof(struct mitsume_ptr) * MITSUME_TEST_ONE - 8;
    int client_id = get_client_id(thread_metadata->local_ctx_clt);
    struct thread_local_inf *local_inf = thread_metadata->local_inf;
    int target_write_set, target_read_set;
    char read_string[MITSUME_TEST_WORKLOAD_NAME_LEN];
...
{
    //mitsume_test_thread(MITSUME_CLT_TEST_THREAD_NUM, local_ctx_clt, &mitsume_test_open);
    //mitsume_test_thread(MITSUME_CLT_TEST_THREAD_NUM, local_ctx_clt, &mitsume_test_read);
    //mitsume_test_thread(MITSUME_CLT_TEST_THREAD_NUM, local_ctx_clt, &mitsume_test_correctness);
    //mitsume_test_thread(MITSUME_CLT_TEST_THREAD_NUM, local_ctx_clt, &mitsume_test_distribution);
