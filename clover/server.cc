...
struct mitsume_ctx_con *server_init(struct configuration_params *input_arg)
{
    ...
    server_ctx->controller_id = get_controller_id(input_arg);//should be get controller_id
...
int server_get_shortcut(struct mitsume_ctx_con *server_ctx)
{
    ...
    for(memory_id=0;memory_id<MITSUME_MEM_NUM;memory_id++)
    {
        ...
        MITSUME_PRINT("%d %llu\n", current_index, (unsigned long long int)end_allocation-start_allocation+1);
...
int server_get_memory(struct mitsume_ctx_con *server_ctx)
{
    ...
    {
        memset(memcached_string, 0, MEMCACHED_MAX_NAME_LEN);
        sprintf(memcached_string, MITSUME_MEMCACHED_MEMORY_ALLOCATION_STRING, per_mem);
        tmp_attr = memcached_get_published_mr(memcached_string);
        memcpy(&memory_attr[per_mem], tmp_attr, sizeof(ptr_attr));
        free(tmp_attr);
        MITSUME_PRINT("%d-(%llx, %ld)\n", per_mem, (unsigned long long)memory_attr[per_mem].addr, (long)memory_attr[per_mem].rkey);
    }
    server_ctx->all_memory_attr = memory_attr;
    return MITSUME_SUCCESS;
}

int server_setup_post_recv(struct configuration_params *input_arg, struct mitsume_ctx_con *context)
{
    void *alloc_space;
...
void* main_server(void *arg)
{
    ...
    //dbg_printf("This is %s %d %d: total %d\n\n", DBG_STRING, machine_id, thread_id, total_machines);
    ...
    if(input_arg->local_thread_id == 0)
    {
        ...
        CPE(server_setup_post_recv(input_arg, server_ctx), "fail to setup post_recv\n", 0);
