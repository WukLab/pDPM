...
struct mitsume_ctx_clt *client_init(struct configuration_params *input_arg)
{
    ...
    i=0;
    {
        MITSUME_PRINT("%llx %llx\n", (unsigned long long int)client_ctx->thread_metadata[i].local_inf->user_input_space[0], (unsigned long long int)client_ctx->thread_metadata[i].local_inf->user_input_mr[0]->lkey);
    }
    return client_ctx;
}
...
int client_get_shortcut(struct mitsume_ctx_clt *client_ctx)
{
    ...
    for(per_allocation=0;per_allocation<MITSUME_SHORTCUT_NUM;per_allocation++)
    {
        ...
        sprintf(memcached_string, MITSUME_MEMCACHED_SHORTCUT_STRING, per_allocation);
        ...
        if(per_allocation==0||per_allocation==1023)
            MITSUME_PRINT("%llx, %ld\n", (unsigned long long)shortcut_attr[per_allocation].addr, (long)shortcut_attr[per_allocation].rkey);
    }*/
    ptr_attr *shortcut_attr = new ptr_attr[MITSUME_SHORTCUT_NUM];
    ptr_attr *tmp_attr;
    int memory_id;
    ...
    int current_index = 0;
    //target_shortcut_entry_space = MITSUME_ROUND_UP(MITSUME_SHORTCUT_NUM, MITSUME_MEM_NUM);
    ...
    for(memory_id=0;memory_id<MITSUME_MEM_NUM;memory_id++)
    {
        ...
        tmp_attr = (ptr_attr*)memcached_get_published_size(memcached_string, sizeof(ptr_attr) * (end_allocation-start_allocation+1));
        memcpy(&shortcut_attr[current_index], tmp_attr, sizeof(ptr_attr) * (end_allocation-start_allocation+1));
...
int client_setup_post_recv(struct configuration_params *input_arg, struct mitsume_ctx_clt *context)
{
    void *alloc_space;
    ...
    context->per_qp_mr_attr_list = new ptr_attr*[node_share_inf->num_local_rcqps];
    context->per_post_recv_mr_list = new struct ibv_mr*[node_share_inf->num_local_rcqps];

    //register a memory space for each qp
    for(per_qp=0;per_qp<node_share_inf->num_local_rcqps;per_qp++)
    {
        context->per_qp_mr_attr_list[per_qp] = new ptr_attr[MITSUME_CON_MESSAGE_PER_POST];
        alloc_space = mitsume_malloc(alloc_size * MITSUME_CON_MESSAGE_PER_POST);
        context->per_post_recv_mr_list[per_qp] = ibv_reg_mr(node_share_inf->pd, 
                                alloc_space, alloc_size * MITSUME_CON_MESSAGE_PER_POST, 
        ...
        {
            tmp_attr_ptr[per_msg].addr = (uint64_t)context->per_post_recv_mr_list[per_qp]->addr+(uint64_t)alloc_size*per_msg;
    ...
    for(per_qp=0;per_qp<node_share_inf->num_local_rcqps;per_qp++)
    {
        ib_post_recv_inf *input_inf = new ib_post_recv_inf[MITSUME_CON_MESSAGE_PER_POST];
        for(per_msg=0;per_msg<MITSUME_CON_MESSAGE_PER_POST;per_msg++)
        {
        ...
        ib_post_recv_connect_qp(node_share_inf, input_inf, context->per_qp_mr_attr_list[per_qp], MITSUME_CON_MESSAGE_PER_POST);
        free(input_inf);
    }
...
void* main_client(void *arg)
{
    ...
    CPE(client_setup_post_recv(input_arg, client_ctx), "fail to setup post_recv\n", 0);
    ...
    write_mr = ibv_reg_mr(node_share_inf->pd, test_write, 1024, MITSUME_MR_PERMISSION);
    read_mr = ibv_reg_mr(node_share_inf->pd, test_read, 1024, MITSUME_MR_PERMISSION);
    ...
    wr_id = mitsume_local_thread_get_wr_id(local_inf);
    userspace_one_write(node_share_inf, wr_id, write_mr, 64, &client_ctx->all_shortcut_attr[16], 0);
    userspace_one_poll(node_share_inf, wr_id, &client_ctx->all_shortcut_attr[16]);
    mitsume_local_thread_put_wr_id(local_inf, wr_id);

    memset(test_write, 0x42, 1024);
    wr_id = mitsume_local_thread_get_wr_id(local_inf);
    userspace_one_write(node_share_inf, wr_id, write_mr, 64, &client_ctx->all_shortcut_attr[16], 16);
    userspace_one_poll(node_share_inf, wr_id, &client_ctx->all_shortcut_attr[16]);
    mitsume_local_thread_put_wr_id(local_inf, wr_id);
