...
atomic<long>mitsume_stat_traffic_array_counter[(MITSUME_CON_NUM+MITSUME_CLT_NUM+MITSUME_MEM_NUM)];
...
int mitsume_stat_show(void)
{
    ...
    for(i=0;i<=MITSUME_STAT_LENGTH;i++)
    {
        ...
        else
        {
            ...
            if(print_flag)
                printf("%s: %lu\n", mitsume_stat_text[i], (unsigned long int)mitsume_stat_counter[i].load());
        }
    }
    
    /*for(i=0;i<MITSUME_STAT_ARRAY_LENGTH;i++)
    {
        printf("array-%d: %lu\n", i, (long unsigned int)mitsume_stat_array_counter[i].load());
    }*/
    for(i=0;i<(MITSUME_CON_NUM+MITSUME_CLT_NUM+MITSUME_MEM_NUM);i++)
    {
        printf("bucket-%d: %lu\n", i, (long unsigned int)mitsume_traffic_array_read(i));
    }
    ...
    for(i=0;i<MITSUME_STAT_SUM_ARRAY_LENGTH;i++)
    {
        ...
        {
            printf("%d summation: %lu\n", i, (long unsigned int )mitsume_stat_sum_array_number[i].load());
            printf("%d count: %lu\n", i, (long unsigned int)mitsume_stat_sum_array_counter[i].load());
            printf("%d average: %lu\n", i, (long unsigned int)mitsume_stat_sum_array_number[i].load()/mitsume_stat_sum_array_counter[i].load());
        }
    }
    return 0;
}
...
/**
 * mitsume_stat_array_add: take an index and a input to add on the respected array value
 * return: NULL
 */
...
/**
 * mitsume_stat_array_sub: take an index and a input to add on the respected array value
 * return: NULL
 */
