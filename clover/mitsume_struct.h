...
struct thread_local_inf{
    ...
    struct mitsume_ptr meshptr_list[MITSUME_CLT_COROUTINE_NUMBER][MITSUME_MAX_REPLICATION*MITSUME_MAX_REPLICATION];
    struct ibv_mr meshptr_mr[MITSUME_CLT_COROUTINE_NUMBER];
    struct ibv_mr *ori_meshptr_mr;

    struct mitsume_ptr chase_empty_list[MITSUME_CLT_COROUTINE_NUMBER][MITSUME_MAX_REPLICATION];
    ...
    struct mitsume_ptr write_checking_ptr[MITSUME_CLT_COROUTINE_NUMBER][MITSUME_MAX_REPLICATION];
...
struct mitsume_gc_entry
{
        ...
        //2. it would be used as a pointer to save update request which will be used to send-out
...
struct mitsume_gc_hashed_entry
{
        ...
        int submitted_epoch;
        //this entry is used with two different ways
        //1. used as a communication flag between allocator and gc thread during recycling
...
struct mitsume_allocator_metadata
{
        ...
        mutex allocator_lock_branch[MITSUME_NUM_REPLICATION_BUCKET][MITSUME_CON_ALLOCATOR_SLAB_NUMBER];
        //struct mitsume_allocator_entry **allocator_node_branch;
        deque<uint64_t> allocator_node_branch[MITSUME_NUM_REPLICATION_BUCKET][MITSUME_CON_ALLOCATOR_SLAB_NUMBER];
...
struct mitsume_ctx_con
{
        ...
        struct mitsume_allocator_metadata thread_metadata[MITSUME_CON_ALLOCATOR_THREAD_NUMBER];
        int controller_id;
        int machine_id;
        ...
        ptr_attr *total_entry_attr;//this is divided from all_memory_attr based on different size
        ...
        struct mitsume_controller_gc_thread_metadata gc_thread_metadata[MITSUME_CON_GC_THREAD_NUMBER];
        pthread_t gc_thread[MITSUME_CON_GC_THREAD_NUMBER];
        queue<struct mitsume_gc_hashed_entry*>public_gc_bucket[MITSUME_CON_GC_THREAD_NUMBER];
...
{
        //deque<struct mitsume_consumer_entry_internal> consumer_node_branch[MITSUME_NUM_REPLICATION_BUCKET][MITSUME_CON_ALLOCATOR_SLAB_NUMBER];
        queue<uint64_t> consumer_node_branch[MITSUME_NUM_REPLICATION_BUCKET][MITSUME_CON_ALLOCATOR_SLAB_NUMBER];
        ...
        std::unordered_map<uint64_t, struct mitsume_private_hash_struct*> mitsume_private_hash;
...
struct mitsume_consumer_gc_metadata
{
        ...
        queue<struct mitsume_consumer_gc_shortcut_update_element*> internal_shortcut_buffer_list[MITSUME_CON_NUM];
        queue<struct mitsume_consumer_gc_shortcut_update_element*> internal_shortcut_send_list[MITSUME_CON_NUM];
...
struct mitsume_ctx_clt
{
        ...
        struct mitsume_consumer_gc_metadata gc_thread_metadata[MITSUME_CLT_CONSUMER_GC_THREAD_NUMS];
        queue<struct mitsume_gc_thread_request *>gc_processing_queue[MITSUME_CLT_CONSUMER_GC_THREAD_NUMS];
        ...
        //DECLARE_HASHTABLE(MITSUME_TOOL_QUERY_HASHTABLE, MITSUME_TOOL_QUERY_HASHTABLE_SIZE_BIT);
        //spinlock_t MITSUME_TOOL_QUERY_HASHTABLE_LOCK[1<<MITSUME_TOOL_QUERY_HASHTABLE_SIZE_BIT];
};
#define MITSUME_DANGEROUS_EPOCH (thread_metadata->local_ctx_clt->gc_current_epoch - MITSUME_GC_EPOCH_THRESHOLD)
...
struct mitsume_tool_communication
{
        ...
        struct mitsume_ptr gc_ptr[MITSUME_MAX_REPLICATION];// this is used for nonlatest&GC in read only
...
struct mitsume_msg
{
        struct mitsume_msg_header msg_header;//dont change this position . it has to be aligned with mitsume_msg msg_header
        union
        {
                struct mitsume_msg_init msg_init; //small
                struct mitsume_msg_entry msg_entry;//use for allocation and garbage collection
                struct mitsume_entry_request msg_entry_request;//use for open specifically //small
                struct mitsume_gc_epoch_forward msg_gc_epoch_forward; //small
                struct mitsume_msg_gc_control msg_gc_control;
                struct mitsume_misc_request_struct msg_misc_request;//for misc-usage, such as wrap version, migration request
...
struct mitsume_large_msg//this is a large message
{
        ...
        union
        {
                ...
                struct mitsume_entry_request msg_entry_request[MITSUME_GC_MAX_BACK_UPDATE_PER_ENTRY];
        }content;

        uint64_t option;
};

#define MITSUME_MAX_MESSAGE_SIZE (MITSUME_MAX(sizeof(struct mitsume_msg), sizeof(struct mitsume_large_msg)))
...
enum MITSUME_MSG_OPTION
{
        MITSUME_ACK_FAIL = 1,
        MITSUME_ACK_SUCCESS = 2
};
...
struct mitsume_hash_struct
{
        ...
        struct mitsume_ptr ptr[MITSUME_MAX_REPLICATION];//0307-modify change to ptr[MITSUME_MAX_REPLICATION]
        struct mitsume_ptr shortcut_ptr;
        //%struct hlist_node hlist;
...
struct mitsume_private_hash_struct
{
        ...
        struct mitsume_ptr ptr[MITSUME_MAX_REPLICATION];
        struct mitsume_ptr shortcut_ptr;
        int access_once; //this number is for nonlatest access only which can save one useless tracing
...
#define MITSUME_TOOL_PRINT_POINTER_KEY_REPLICATION(entry, factor, key) \
        mitsume_tool_print_pointer_key_replication(entry, factor, key, __LINE__, __func__)

#define MITSUME_STRUCT_CHECKNULL_PTR_REPLICATION(src, replication_factor) \
        mitsume_struct_checknull_ptr_replication(src, replication_factor, __LINE__, __func__)

#define MITSUME_STRUCT_CHECKEQUAL_PTR_REPLICATION(des, src, replication_factor) \
        mitsume_struct_checkequal_ptr_replication(des, src, replication_factor, __LINE__, __func__)
