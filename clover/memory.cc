...
int memory_allocate_shortcut(struct configuration_params *input_arg)
{
    ...
    shortcut_space = mitsume_malloc(target_shortcut_entry_space * sizeof(mitsume_shortcut));
    assert(shortcut_space);
    MITSUME_PRINT("allocate %lluB (%d-%d) for %d shortcut(%d)\n", 
        (long long unsigned int)target_shortcut_entry_space * sizeof(mitsume_shortcut),
        start_allocation, end_allocation,
        (int)target_shortcut_entry_space,
        (int)sizeof(mitsume_shortcut));
    shortcut_mr = ibv_reg_mr(node_share_inf->pd, 
        shortcut_space, target_shortcut_entry_space * sizeof(mitsume_shortcut), 
        MITSUME_MR_PERMISSION);
    assert(shortcut_mr);
    for(per_allocation = start_allocation;per_allocation<=end_allocation;per_allocation++)
    {
        ptr_attr shortcut_attr = {
            addr: (uint64_t)shortcut_space + ((per_allocation-start_allocation)*sizeof(mitsume_shortcut)),
            rkey: shortcut_mr->rkey,
            machine_id: (short)input_arg->machine_id
        };
        memcpy(&output_shortcut_attr[per_allocation-start_allocation], &shortcut_attr, sizeof(ptr_attr));
    }
    memset(memcached_string, 0, MEMCACHED_MAX_NAME_LEN);
    sprintf(memcached_string, MITSUME_MEMCACHED_SHORTCUT_STRING, memory_id);
    memcached_publish(memcached_string, output_shortcut_attr, sizeof(ptr_attr)*(end_allocation-start_allocation+1));
    /*for(per_allocation = start_allocation;per_allocation<=end_allocation;per_allocation++)
    {
        ptr_attr shortcut_attr = {
            addr: (uint64_t)shortcut_space + ((per_allocation-start_allocation)*sizeof(mitsume_shortcut)),
        ...
        memcached_publish(memcached_string, &shortcut_attr, sizeof(ptr_attr));
    }*/
...
int memory_allocate_memoryspace(struct configuration_params *input_arg)
{
    ...
    allocate_space = mitsume_malloc((unsigned long long)MITSUME_MEMORY_PER_ALLOCATION_KB*1024);
    assert(allocate_space);
    MITSUME_PRINT("allocate %lluB memory\n", (unsigned long long)MITSUME_MEMORY_PER_ALLOCATION_KB*1024);
    allocate_mr = ibv_reg_mr(node_share_inf->pd, 
        allocate_space, (long long int)MITSUME_MEMORY_PER_ALLOCATION_KB*1024, 
        MITSUME_MR_PERMISSION);
    assert(allocate_mr);
    ...
    memset(memcached_string, 0, MEMCACHED_MAX_NAME_LEN);
    sprintf(memcached_string, MITSUME_MEMCACHED_MEMORY_ALLOCATION_STRING, memory_id);
    memcached_publish(memcached_string, &allocate_attr, sizeof(ptr_attr));
    MITSUME_PRINT("%llx, %ld\n", (unsigned long long)allocate_attr.addr, (long)allocate_attr.rkey);
    return MITSUME_SUCCESS;
}
