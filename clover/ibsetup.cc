...
int p15_ib_post_recv_ud_qp(struct ib_inf *inf, int udqp_index, int post_recv_base, int post_recv_num)
{
    int i, count=0;
    ...
    {
        dbg_printf("[%s] post too big nums as %d (%d)\n", __func__, post_recv_num + post_recv_base, RSEC_CQ_DEPTH);
    ...
    for(i=post_recv_base;i<post_recv_num+post_recv_base;i++)
    {
        ...
        //if(i==0)
        //      dbg_printf("[%s] %lx %lx %lx\n", __func__, sge[0].addr, (long unsigned int)sge[0].length, (long unsigned int)sge[0].lkey);
        input_wr.next = NULL;
        input_wr.sg_list = sge;
        input_wr.wr_id = ((uint64_t)i<<P15_UD_POST_RECV_ID_SHIFT) + (uint64_t)sge[0].addr;
        input_wr.num_sge = 1;
...
struct ibv_device* ib_get_device(struct ib_inf *inf, int port)
{
    ...
    if(num_devices==0)//assuming we only have one device now, need to modify this part later
    ...
    i=inf->device_id;
    {
        ...
        if(device_attr.phys_port_cnt<port)
            die_printf("%s: port not enough %d:%d\n", __func__, port, device_attr.phys_port_cnt);
...
void ib_create_udqps(struct ib_inf *inf)
    /*
       ...
       2. dgram_buf[num_local_udpqs][UD_CQ_DEPTH]: malloc all elements and register memory
       */
{
    int i,j;
    ...
    for(i = 0; i < inf->num_local_udqps; i++) {
        ...
        inf->dgram_send_cq[i] = ibv_create_cq(inf->ctx,
                RSEC_CQ_DEPTH, NULL, NULL, 0);
        assert(inf->dgram_send_cq[i] != NULL);

        inf->dgram_recv_cq[i] = ibv_create_cq(inf->ctx,
                RSEC_CQ_DEPTH, NULL, NULL, 0);
        ...
        //dbg_printf("[%s] %lx %lx\n", __func__, (long unsigned int)inf->dgram_send_cq[i], (long unsigned int)inf->dgram_recv_cq[i]);
        ...
        inf->dgram_buf_mr[i] = (struct ibv_mr **)malloc(sizeof(struct ibv_mr *) * RSEC_CQ_DEPTH);
        for(j=0;j<RSEC_CQ_DEPTH;j++)
        {
            inf->dgram_buf[i][j] = p15_malloc(sizeof(struct P15_message_frame) + UD_SHIFT_SIZE);
            assert(inf->dgram_buf[i][j]!=NULL);
            inf->dgram_buf_mr[i][j] = ibv_reg_mr(inf->pd, inf->dgram_buf[i][j], sizeof(struct P15_message_frame) + UD_SHIFT_SIZE, IBV_ACCESS_LOCAL_WRITE|IBV_ACCESS_REMOTE_WRITE|IBV_ACCESS_REMOTE_READ);
...
{
    int i;
    assert(inf->conn_qp != NULL &&
            inf->conn_cq != NULL && inf->pd != NULL && inf->ctx != NULL);
    assert(inf->num_local_rcqps >= 1 && inf->dev_port_id >= 1);
    inf->server_recv_cq = ibv_create_cq(inf->ctx, RSEC_CQ_DEPTH * inf->num_local_rcqps, NULL, NULL, 0);
...
struct ib_inf* ib_setup(int id, int port, int num_rcqp_to_server, int num_rcqp_to_client, int num_udqps, int num_loopback, int total_machines, int device_id, int role_int)
{
    /*
    ...
    assert(num_rcqp_to_server >= 0 && num_rcqp_to_client >0);//at least one client/one memory
    ...
    inf->all_rcqps = (struct ib_qp_attr **) malloc(inf->num_global_rcqps * sizeof(struct ib_qp_attr *));

    inf->rcqp_buf = (uint64_t *)malloc(sizeof(uint64_t *) * inf->num_local_rcqps);
    inf->rcqp_buf_mr = (struct ibv_mr**)malloc(sizeof(struct ibv_mr **) * inf->num_local_rcqps);


    inf->all_udqps = (struct ib_qp_attr **) malloc(inf->num_global_udqps * sizeof(struct ib_qp_attr *));
    inf->dgram_ah = (struct ibv_ah **) malloc(inf->num_global_udqps * sizeof(struct ibv_ah *));

    inf->dgram_buf = (void***)malloc(sizeof(void **) * inf->num_local_udqps);
    inf->dgram_buf_mr = (struct ibv_mr ***)malloc(sizeof(struct ibv_mr **) * inf->num_local_udqps);
    ...
    if(inf->num_local_udqps >= 1) {
        inf->dgram_qp = (struct ibv_qp **)
            malloc(inf->num_local_udqps * sizeof(struct ibv_qp *));
        inf->dgram_send_cq = (struct ibv_cq **)
            malloc(inf->num_local_udqps * sizeof(struct ibv_cq *));
        inf->dgram_recv_cq = (struct ibv_cq **)
            malloc(inf->num_local_udqps * sizeof(struct ibv_cq *));
    ...
    if(inf->num_local_rcqps >= 1) {
        inf->conn_qp = (struct ibv_qp **)
            malloc(inf->num_local_rcqps * sizeof(struct ibv_qp *));
        inf->conn_cq = (struct ibv_cq **)
            malloc(inf->num_local_rcqps * sizeof(struct ibv_cq *));
        assert(inf->conn_qp != NULL && inf->conn_cq != NULL);
        ib_create_rcqps(inf, role_int);
    }
    //Create counter
    inf->ud_qp_counter = (uint64_t*)malloc(sizeof(uint64_t) * inf->num_local_udqps);
    memset(inf->ud_qp_counter, 0, sizeof(uint64_t) * inf->num_local_udqps);
    inf->rc_qp_counter = (uint64_t*)malloc(sizeof(uint64_t) * inf->num_local_rcqps);
...
int ib_post_recv_connect_qp(struct ib_inf *context, ib_post_recv_inf *post_recv_inf_list, struct ib_mr_attr *input_mr_array, int input_mr_array_length)
{
    int i;
    int ret;
    for(i=0;i<input_mr_array_length;i++)//need optimization to remove multiple post-recves
    {
        ...
        recv_wr.wr_id = RSEC_ID_COMBINATION(post_recv_inf_list[i].qp_index, post_recv_inf_list[i].mr_index);
        ...
        ret = ibv_post_recv(context->conn_qp[post_recv_inf_list[i].qp_index], &recv_wr, &bad_wr);
...
struct ib_inf *ib_complete_setup(struct configuration_params *input_arg, int role_int, const char *role_str)
{
    int machine_id;
    ...
    total_machines = input_arg->num_servers+input_arg->num_clients+input_arg->num_memorys;
    machine_id = input_arg->machine_id;
    node_share_inf = ib_setup(
            input_arg->machine_id,
            input_arg->base_port_index, 
            input_arg->num_servers * P15_PARALLEL_RC_QPS, 
            (input_arg->num_clients+ input_arg->num_memorys)* P15_PARALLEL_RC_QPS,
            P15_PARALLEL_UD_QPS,
            input_arg->num_loopback,
            total_machines,
            input_arg->device_id,
            role_int);
    ...
    for(cumulative_id=0;cumulative_id<total_machines;cumulative_id++)
    {
        for(i=0;i<node_share_inf->num_local_rcqps;i++)
        {
            ...
            node_share_inf->all_rcqps[total_qp_count] = memcached_get_published_qp(srv_name);
            total_qp_count++;
        }
    ...
    {	
        for(j=0;j<P15_PARALLEL_RC_QPS;j++)
        {
            ...
            int target_qp_num = i*total_machines*P15_PARALLEL_RC_QPS+machine_id*P15_PARALLEL_RC_QPS+j;
            /*dbg_printf(
    ...
    for(cumulative_id=0;cumulative_id<total_machines;cumulative_id++)
    {
        for(i=0;i<node_share_inf->num_local_udqps;i++)
        {
            ...
            node_share_inf->all_udqps[total_qp_count] = memcached_get_published_qp(srv_name);
            total_qp_count++;
        }
    ...
    {	
        //dbg_printf("UD: list %d: %d %d %d\n", i, node_share_inf->all_udqps[i]->lid, node_share_inf->all_udqps[i]->qpn, node_share_inf->all_udqps[i]->sl);
        node_share_inf->dgram_ah[i] = ib_create_ah_for_ud(node_share_inf, i, node_share_inf->all_udqps[i]);
    }*/
       ...
       */
{
    ...
    attr.path_mtu = (RSEC_NETWORK_MODE == RSEC_NETWORK_ROCE) ? IBV_MTU_1024 : IBV_MTU_4096;
    ...
    if (RSEC_NETWORK_MODE == RSEC_NETWORK_ROCE)
    {
        //attr.ah_attr.grh.dgid.global.interface_id = dest->remote_gid.global.interface_id;
        //attr.ah_attr.grh.dgid.global.subnet_prefix = dest->remote_gid.global.subnet_prefix;
...
struct ibv_ah *ib_create_ah_for_ud(struct ib_inf *inf, int ah_index, struct ib_qp_attr *dest)
{
...
inline int hrd_poll_cq(struct ibv_cq* cq, int num_comps, struct ibv_wc* wc, int sleep_flag)
{
  int comps = 0;
...
void userspace_wait_wr_table_value(uint64_t wr_id)
{
    ...
    {
        MITSUME_PRINT_ERROR("wait %llu:%llx\n", (unsigned long long int)wr_id, (unsigned long long int)WR_ID_WAITING_TABLE[wr_id]);
...
int userspace_one_send(struct ibv_qp *qp, 
                   ...
                   uint64_t wr_id)
{
        ...
        if(ret)
            die_printf("%s-%d: ibv_post_send fail %llu\n", __func__, __LINE__, (unsigned long long int)wr.wr_id);
                    ...
                    struct ib_mr_attr *remote_mr, unsigned long long offset)
{
        ...
        //MITSUME_PRINT("%llu:%llx:%llx\n", (unsigned long long int)wr.wr_id, (unsigned long long int) test_sge.addr, (unsigned long long int) test_sge.lkey);
        ...
        if(ret)
            die_printf("%s-%d: ibv_post_send fail %d:%llu\n", __func__, __LINE__, ret, (unsigned long long int)wr.wr_id);
...
int userspace_one_cs(struct ib_inf *ib_ctx, uint64_t wr_id,
                    ...
                    unsigned long long guess_value, unsigned long long set_value)
{
        struct ibv_sge test_sge;
        struct ibv_send_wr wr, *bad_send_wr;
        int ret;
                    ...
                    struct ibv_sge *input_sge, int input_sge_length)
{
        ...
        wr.wr.rdma.rkey = remote_mr->rkey;
        //for(int i=0;i<input_sge_length;i++)
        //    MITSUME_PRINT("%d:%llx:%llx\n", i, (unsigned long long int) input_sge[i].addr, (unsigned long long int) input_sge[i].lkey);
        //ret = ibv_post_send(ib_ctx->conn_qp[remote_mr->machine_id], &wr, &bad_send_wr);
        int qp_idx = wr_id_to_qp_index(wr_id, remote_mr->machine_id);
        ret = ibv_post_send(ib_ctx->conn_qp[qp_idx], &wr, &bad_send_wr);
        if(ret)
...
int userspace_one_poll(struct ib_inf *ib_ctx, uint64_t wr_id, struct ib_mr_attr *remote_mr)
{
        ...
        while (comps < num_comps)
        {
            if(userspace_check_done_wr_table(wr_id))
                break;
            //int new_comps = ibv_poll_cq(ib_ctx->conn_cq[remote_mr->machine_id], num_comps - comps, &wc[comps]);
            int qp_idx = wr_id_to_qp_index(wr_id, remote_mr->machine_id);
            int new_comps = ibv_poll_cq(ib_ctx->conn_cq[qp_idx], num_comps - comps, &wc[comps]);
            count++;
            if (new_comps != 0) 
            {
            ...
            // Ideally, we should check from comps -> new_comps - 1 
                if (wc[comps].status != 0)
                {
                    fprintf(stderr, "Bad wc status %d %llu\n", wc[comps].status, (unsigned long long int)wc[comps].wr_id);
                    exit(0);
                    return 1;
                    //exit(0);
                }
...
int userspace_one_poll_wr(struct ibv_cq *cq, int tar_mem, struct ibv_wc *input_wc, int sleep_flag)
...
int userspace_refill_used_postrecv(struct ib_inf *ib_ctx, ptr_attr **per_qp_mr_attr_list, struct ibv_wc *input_wc, int length, uint32_t data_length)
{
    int per_wc;
...
int mitsume_send_full_message(struct ib_inf *ib_ctx, struct thread_local_inf *local_inf, struct ibv_mr *send_mr, struct ibv_mr *recv_mr, struct mitsume_msg_header *header_ptr, int source_machine, int target_machine, uint32_t send_size)
{
    //verify send message
    ...
    //send out message
    //userspace_one_send(ib_ctx->conn_qp[target_machine], send_mr, send_size, wr_id);
    ...
    while(check_msg_ptr->end_crc != MITSUME_REPLY_CRC);//wait for reply
    
    return MITSUME_SUCCESS;
}

int mitsume_send_full_message_async(struct ib_inf *ib_ctx, struct thread_local_inf *local_inf, struct ibv_mr *send_mr, struct ibv_mr *recv_mr, struct mitsume_msg_header *header_ptr, int source_machine, int target_machine, uint32_t send_size, int coro_id, coro_yield_t &yield)
{
    //verify send message
    uint64_t wr_id = mitsume_local_thread_get_wr_id(local_inf);
                    ...
                    struct ibv_mr *local_mr, int reply_size)
{
    ...
    //MITSUME_PRINT("send back to %d %llx %llx\n", recv_header_ptr->reply_attr.machine_id, (unsigned long long int)wr.wr.rdma.remote_addr, (unsigned long long int)wr.wr.rdma.rkey);
    //ret = ibv_post_send(ib_ctx->conn_qp[recv_header_ptr->reply_attr.machine_id], &wr, &bad_send_wr);
