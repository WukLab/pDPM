...
void mitsume_test_read_ycsb(const char *input_string, int **op_key, uint64_t **target_key)
{       
...
void mitsume_benchmark_slave_func(coro_yield_t &yield, tuple<struct mitsume_consumer_metadata *, int, int*, mitsume_key *, int *, long *>id_tuple)
{
    ...
    while(!end_flag)
    {
        ...
        *next_index = *next_index + 1;
        ...
        if(op_key[i]==0)
        {
            ret = mitsume_tool_read(thread_metadata, key, read, &read_size, MITSUME_TOOL_KVSTORE_READ, coro_id, yield);
            if(MITSUME_YCSB_VERIFY_LEVEL)
            {
                if(read[0]!=write[0])
        ...
        {
            //ret = mitsume_tool_write(thread_metadata, key, write, test_size, MITSUME_TOOL_KVSTORE_WRITE);
            ret = mitsume_tool_write(thread_metadata, key, write, test_size, MITSUME_TOOL_KVSTORE_WRITE, coro_id, yield);
        }
...
void mitsume_benchmark_master_func(coro_yield_t &yield, tuple<struct mitsume_consumer_metadata *, int, int*, mitsume_key *, int *, long *>id_tuple)
{
    struct mitsume_consumer_metadata *thread_metadata = get<0>(id_tuple);
    int coro_id = get<1>(id_tuple);
    long *local_op = get<5>(id_tuple);
...
void *mitsume_benchmark_coroutine(void *input_metadata)
{
    ...
    struct mitsume_consumer_metadata *thread_metadata = (struct mitsume_consumer_metadata *)input_metadata;
    struct thread_local_inf *local_inf = thread_metadata->local_inf;
    ...
    target_set = thread_metadata->thread_id + (client_id)*MITSUME_BENCHMARK_THREAD_NUM;
    ...
    if(thread_metadata->thread_id == 0)
    {
        if(client_id==0)
        {
            ...
            for(i=1;i<=key_range;i++)
            {
                key = i;
                memset(write, 0x31+(key%30), MITSUME_BENCHMARK_SIZE);
                ret = mitsume_tool_open(thread_metadata, key, write, MITSUME_BENCHMARK_SIZE, MITSUME_BENCHMARK_REPLICATION);
                if(ret)
    ...
    for(coro_i=0;coro_i<MITSUME_YCSB_COROUTINE;coro_i++)
    {
        ...
        {
            local_inf->coro_arr[coro_i] = coro_call_t(bind(mitsume_benchmark_master_func, _1, make_tuple(thread_metadata, coro_i, op_key, target_key, &share_index, &local_op)), attributes(fpu_not_preserved));
        }
        else
        {
            local_inf->coro_queue.push(coro_i);
            local_inf->coro_arr[coro_i] = coro_call_t(bind(mitsume_benchmark_slave_func, _1, make_tuple(thread_metadata, coro_i, op_key, target_key, &share_index, &local_op)), attributes(fpu_not_preserved));
        }
    }
...
void *mitsume_benchmark_ycsb(void *input_metadata)
{
    ...
    while(!end_flag)
    {
        ...
        if(op_key[i]==0)
        {
            mitsume_tool_read(thread_metadata, key, read, &read_size, MITSUME_TOOL_KVSTORE_READ);
            if(MITSUME_YCSB_VERIFY_LEVEL)
                if(read[0]!=write[0])
                    MITSUME_PRINT("doesn't match %c %c\n", read[0], write[0]);
        }
...
void *mitsume_benchmark_latency(void *input_metadata)
{
    ...
    MITSUME_PRINT("------\n");
    ret = mitsume_tool_open(thread_metadata, key, write, 36, MITSUME_BENCHMARK_REPLICATION);
    for(j=0;j<test_size.size();j++)
    {
        before = get_current_ns();
        for(i=0;i<MITSUME_BENCHMARK_TIME;i++)
        {
            ret = mitsume_tool_write(thread_metadata, key, write, test_size[j]-8*MITSUME_BENCHMARK_REPLICATION, MITSUME_TOOL_KVSTORE_WRITE);
            if(ret!=MITSUME_SUCCESS)
                MITSUME_INFO("error %lld %d\n", (unsigned long long int)key, ret);
        }
        after = get_current_ns();
        cout<<test_size[j]<<": average write time(us):"<<(after-before).count()/MITSUME_BENCHMARK_TIME<<endl;
        
        before = get_current_ns();
        for(i=0;i<MITSUME_BENCHMARK_TIME;i++)
        {
            ret = mitsume_tool_read(thread_metadata, key, read, &read_size, MITSUME_TOOL_KVSTORE_READ);
            if(ret!=MITSUME_SUCCESS)
                MITSUME_INFO("error %lld %d\n", (unsigned long long int)key, ret);
        }
        after = get_current_ns();
        cout<<test_size[j]<<": average read time(us):"<<(after-before).count()/MITSUME_BENCHMARK_TIME<<endl;
    }
...
void *mitsume_benchmark_load(void *input_metadata)
{
    ...
    if(thread_metadata->thread_id == 0)
    {
        ...
        {
            MITSUME_PRINT("------\n");
            mitsume_tool_open(thread_metadata, MITSUME_TEST_LOAD_DUMMY, write, test_size, 1);
            mitsume_tool_open(thread_metadata, MITSUME_TEST_LOAD_DUMMY+1, write, test_size, 1);
            mitsume_tool_open(thread_metadata, MITSUME_TEST_LOAD_READ_R1, write, test_size, 1);
            mitsume_tool_open(thread_metadata, MITSUME_TEST_LOAD_DUMMY+2, write, test_size, 1);
            mitsume_tool_open(thread_metadata, MITSUME_TEST_LOAD_DUMMY+3, write, test_size, 1);
            mitsume_tool_open(thread_metadata, MITSUME_TEST_LOAD_READ_R2, write, test_size, 3);
    ...
    while(!end_flag)
    {
        ...
        {
            mitsume_tool_read(thread_metadata, MITSUME_TEST_LOAD_READ_R1, read, &read_size, MITSUME_TOOL_KVSTORE_READ);
        }
        else if(read_thread_id >=0)
        {
            //mitsume_tool_read(thread_metadata, MITSUME_TEST_LOAD_READ_R1, read, &read_size, MITSUME_TOOL_KVSTORE_READ);
            mitsume_tool_read(thread_metadata, MITSUME_TEST_LOAD_READ_R2, read, &read_size, MITSUME_TOOL_KVSTORE_READ);
        }
        else
        {
            ret = mitsume_tool_write(thread_metadata, MITSUME_TEST_LOAD_WRITE_START+write_thread_id, write, test_size, MITSUME_TOOL_KVSTORE_WRITE);
            if(ret!=MITSUME_SUCCESS)
                MITSUME_INFO("error %lld %d\n", (unsigned long long int)write_thread_id, ret);
        }
        local_op++;
    }
...
int mitsume_benchmark_thread(int thread_num, struct mitsume_ctx_clt *local_ctx_clt, void* (*fun_ptr)(void *input_metadata))
{
    int i=0;
    total_op = 0;
    mitsume_sync_barrier_init(thread_num, get_client_id(local_ctx_clt), MITSUME_CLT_NUM);
    ...
    {
        pthread_create(&thread_job[i], NULL, fun_ptr, &local_ctx_clt->thread_metadata[i]);
    }
    while(ready_flag == 0);
    start_flag = 1;
    ...
    cout<<fixed<<"throughput "<<((float)total_op.load()/(after-before).count())*1000<<" /seconds"<<endl;
    //mitsume_stat_show();
...
int mitsume_benchmark(struct mitsume_ctx_clt *local_ctx_clt)
{
    ...
    //mitsume_benchmark_thread(MITSUME_BENCHMARK_THREAD_NUM, local_ctx_clt, &mitsume_benchmark_coroutine);
    //mitsume_benchmark_thread(MITSUME_TEST_LOAD_WRITE_NUM+MITSUME_TEST_LOAD_READ_NUM, local_ctx_clt, &mitsume_benchmark_load);
    return 0;
}
