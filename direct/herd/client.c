...
int userspace_one_write(unsigned long long tar_key, int tar_mem, struct hrd_ctrl_blk* mem_cb, 
                   struct ibv_mr *mitsume_mr, int request_size,
                   struct hrd_qp_attr ***mr_qp_list, int clt_gid)
{
...
int userspace_one_read(unsigned long long tar_key, int tar_mem, struct hrd_ctrl_blk* mem_cb, 
                   struct ibv_mr *mitsume_mr, int request_size,
                   struct hrd_qp_attr ***mr_qp_list, int verification, int clt_gid)
{
        struct ibv_sge test_sge;
        struct ibv_send_wr wr, *bad_send_wr;
        int ret;
...
int userspace_one_compare_and_swp(unsigned long long tar_key, int tar_mem, 
                ...
                uint64_t expect_value, uint64_t assign_value, int clt_gid)
{
        struct ibv_sge test_sge;
        struct ibv_send_wr wr, *bad_send_wr;
        int ret;
        ...
        *write_lock = lock;
        ...
        do
        {
                *read_lock = 0xffffffff;
                userspace_one_compare_and_swp(tar_key, tar_mem, mem_cb, input_mr, mr_qp_list, unlock, lock, clt_gid);
                userspace_one_poll(mem_cb, tar_mem);
                count++;
        }while(*read_lock!=unlock);
        
        /*// 2. read from space
        userspace_one_read(tar_key, tar_mem, mem_cb, input_mr, request_size+16, mr_qp_list, 0, clt_gid);
        userspace_one_poll(mem_cb, tar_mem);*/

        // 3. write to backup
        for(j=0;j<NUM_REPLICATION;j++)
                userspace_one_write(tar_key, (tar_mem+j)%NUM_MEMORY, mem_cb, output_mr, request_size, backupmr_qp_list, clt_gid);
        for(j=0;j<NUM_REPLICATION;j++)
        ...
        for(j=0;j<NUM_REPLICATION;j++)
                userspace_one_read(tar_key, (tar_mem+j)%NUM_MEMORY, mem_cb, input_mr, request_size, backupmr_qp_list, 1, clt_gid);
        for(j=0;j<NUM_REPLICATION;j++)
                userspace_one_poll(mem_cb, (tar_mem+j)%NUM_MEMORY);
        #endif
        ...
        for(j=0;j<NUM_REPLICATION;j++)
                userspace_one_write(tar_key, (tar_mem+j)%NUM_MEMORY, mem_cb, output_mr, request_size+16, mr_qp_list, clt_gid);
        for(j=0;j<NUM_REPLICATION;j++)
                userspace_one_poll(mem_cb, (tar_mem+j)%NUM_MEMORY);
        ...
        for(j=0;j<NUM_REPLICATION;j++)
                userspace_one_read(tar_key, (tar_mem+j)%NUM_MEMORY, mem_cb, input_mr, request_size+16, mr_qp_list, 1, clt_gid);
        for(j=0;j<NUM_REPLICATION;j++)
                userspace_one_poll(mem_cb, (tar_mem+j)%NUM_MEMORY);
        #endif
        ...
        // 5. write to unlock
        *write_lock = unlock;
        userspace_one_write(tar_key, tar_mem, mem_cb, output_mr, 8, mr_qp_list, clt_gid);
        userspace_one_poll(mem_cb, tar_mem);
        return 0;
}


int mitsume_read_one_key_post(int tar_key, 
        struct hrd_ctrl_blk *mem_cb, 
        struct hrd_qp_attr ***mr_qp_list,
        struct hrd_qp_attr ***backupmr_qp_list,
        struct ibv_mr *input_mr, 
        void *addr, int clt_gid)
{
        ...
        if(CRC_MODE)
        {
                do
                {
                        userspace_one_read(tar_key, tar_mem, mem_cb, input_mr, request_size+16, mr_qp_list, 0, clt_gid);
                        userspace_one_poll(mem_cb, tar_mem);
                        //userspace_liteapi_rdma_read(undo_key_map[key].primary_key[0], temp_read, request_size+16, 0, LITE_TEST_PW);
                        
                        crc_check = (uint64_t*)(input_mr->addr+request_size+8);
                        crc_compute = crc32(~0, input_mr->addr+8, request_size);
                        
                        //enable following two lines and disable above two lines can disable read-crc
                        //crc_check = &unlock;
                        //crc_compute = 0;
                        count++;
        ...
        else
        {
                ...
                do
                {
                        *check = 0xffffffff;
                        userspace_one_compare_and_swp(tar_key, tar_mem, mem_cb, input_mr, mr_qp_list, unlock, lock, clt_gid);
                        userspace_one_poll(mem_cb, tar_mem);
                        //userspace_liteapi_compare_swp(undo_key_map[key].primary_key[0], &read, unlock, lock);
                        count++;
                }while(*check!=unlock);
                ...
                // 3. write to unlock
                *check = unlock;
                userspace_one_write(tar_key, tar_mem, mem_cb, input_mr, sizeof(uint64_t), mr_qp_list, clt_gid);
                userspace_one_poll(mem_cb, tar_mem);
                //userspace_liteapi_rdma_write(undo_key_map[key].primary_key[0], &unlock, sizeof(uint64_t), 0, LITE_TEST_PW);
        }
        //userspace_liteapi_rdma_read(key_map[key], addr, request_size, 8, LITE_TEST_PW);
        //if(count>1)
        //        printf("read %d\n", count);
...
int mitsume_lock_and_update_one_key_post(int tar_key, 
        struct hrd_ctrl_blk *mem_cb, 
        struct hrd_qp_attr ***mr_qp_list,
        struct hrd_qp_attr ***backupmr_qp_list,
        struct ibv_mr *input_mr,
        ...
        void *addr, int clt_gid)
{
        ...
        *write_lock = lock;
        ...
        // 4. write to old place
        for(j=0;j<NUM_REPLICATION;j++)
                userspace_one_write(tar_key, (tar_mem+j)%NUM_MEMORY, mem_cb, output_mr, request_size+8, mr_qp_list, clt_gid);
        for(j=0;j<NUM_REPLICATION;j++)
                userspace_one_poll(mem_cb, (tar_mem+j)%NUM_MEMORY);
        ...
        for(per_memory=0;per_memory<NUM_MEMORY;per_memory++)
        {
                ...
                for(per_mr=0;per_mr<=HERD_NUM_KEYS;per_mr++)
                {
                        ...
                        else
                        {
                                ...
                                backupmr_qp[per_mr]->buf_addr = backupmr_qp[per_mr-1]->buf_addr + HERD_SPACE_SIZE;
                        }
                        if(per_mr%50000 == 0)
                        {
                                printf("main: client %d get memory %d from memory %d. \n", clt_gid, per_mr, per_memory);
                        }
                }
        }
...
void* run_client(void* arg) {
  ...
  struct hrd_ctrl_blk *mem_cb;
  struct hrd_qp_attr ***mr_qp_list = malloc(sizeof(struct hrd_qp_attr **) * NUM_MEMORY);
  struct hrd_qp_attr ***backupmr_qp_list = malloc(sizeof(struct hrd_qp_attr **) * NUM_MEMORY);
  mem_cb = mitsume_setup_connection(clt_gid, ib_port_index, mr_qp_list, backupmr_qp_list);
  ...
  struct ibv_mr *input_mr = ibv_reg_mr(mem_cb->pd, input_space, HERD_SPACE_SIZE, IBV_ACCESS_LOCAL_WRITE);
  struct ibv_mr *output_mr = ibv_reg_mr(mem_cb->pd, output_space, HERD_SPACE_SIZE, IBV_ACCESS_LOCAL_WRITE);
  ...
  get_file(&op_key, &write_key, clt_gid);
        ...
        if(clt_gid==0) {
                ...
                for(per_key=1;per_key<=HERD_NUM_KEYS;per_key++) {
                        for(per_memory=0;per_memory<NUM_MEMORY;per_memory++) {
                                //memset
                                userspace_one_write(per_key, per_memory, mem_cb, output_mr, HERD_SPACE_SIZE, mr_qp_list, clt_gid);
                                userspace_one_poll(mem_cb, per_memory);
                        }
                        memset(read_write_addr, 0x31+(per_key)%20, HERD_SPACE_SIZE);
                        mitsume_write_one_key_post(per_key, mem_cb, mr_qp_list, backupmr_qp_list, input_mr, output_mr, read_write_addr, clt_gid);
                }
...
#define _MEASURE_LATENCY_
  ...
  while (1) {
    if (rolling_iter >= K_512) {
      ...
      "send %.2f recv %.2f \n",
      	clt_gid,
             K_512 / seconds, nb_tx, send_traffic[clt_gid]/seconds, recv_traffic[clt_gid]/seconds);
    ...
    if(is_update) {
        ...
        mitsume_lock_and_update_one_key_post(key_i, mem_cb, mr_qp_list, backupmr_qp_list, input_mr, output_mr, read_write_addr, clt_gid);
#else
        mitsume_write_one_key_post(key_i, mem_cb, mr_qp_list, backupmr_qp_list, input_mr, output_mr, read_write_addr, clt_gid);
#endif
    } else {
        mitsume_read_one_key_post(key_i, mem_cb, mr_qp_list, backupmr_qp_list, input_mr, read_write_addr, clt_gid);
    }
	...
	printf("[Thread-ID %2d] Read. NR: %10d Avg: %10lu (ns)\n",
		clt_gid,
		nr_rd, nr_rd ? rd_total / nr_rd : 0);
	printf("[Thread-ID %2d] Writ. NR: %10d Avg: %10lu (ns)\n",
		clt_gid,
		nr_wr, nr_wr ? wr_total / nr_wr : 0);
...
void* run_memory(void* arg) {
        ...
        */
        ...
        struct hrd_ctrl_blk* cb = hrd_ctrl_blk_init(
                ...
                NULL, HERD_SPACE_SIZE*(HERD_NUM_KEYS+1)*2, -1,         /* prealloc conn buf, buf size, key */
                1, DGRAM_BUF_SIZE, -1); /* num_dgram_qps, dgram_buf_size, key */
        ...
        char shared_memory_name[HRD_QP_NAME_SIZE];
        {
                ...
                for(i=0;i<=HERD_NUM_KEYS;i++)
                {
                        ...
                        //shared_memory_region[i] = ibv_reg_mr(cb->pd, shared_memory_space[i], HERD_SPACE_SIZE, IBV_ACCESS_LOCAL_WRITE|IBV_ACCESS_REMOTE_WRITE|IBV_ACCESS_REMOTE_READ|IBV_ACCESS_REMOTE_ATOMIC);
                        //backup_memory_space[i] = malloc(HERD_SPACE_SIZE);
                        backup_memory_space[i] = cb->conn_buf + (HERD_SPACE_SIZE*(i + HERD_NUM_KEYS+1));
                        //memset(backup_memory_space[i], 0, HERD_SPACE_SIZE);
                        //backup_memory_region[i] = ibv_reg_mr(cb->pd, backup_memory_space[i], HERD_SPACE_SIZE, IBV_ACCESS_LOCAL_WRITE|IBV_ACCESS_REMOTE_WRITE|IBV_ACCESS_REMOTE_READ|IBV_ACCESS_REMOTE_ATOMIC);
                ...
                printf("main: Memory %d publish %d mr %s\n", mem_gid, HERD_NUM_KEYS, shared_memory_name);
        }
        #ifdef MICA_IF_TEST_LATENCY
