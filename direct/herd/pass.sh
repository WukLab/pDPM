...
do
        ...
        scp -q -P 456 Makefile master.c worker.c main.h client.c main.c run-machine.sh run-servers.sh run-memory.sh pre-run.sh setup.json pkill.sh $VARI:$path &
        scp -q -P 456 ../mica/mica.h ../mica/mica.c $VARI:$path/../mica/ &
        scp -q -P 456 ../libhrd/hrd_conn.c ../mica/mica.c $VARI:$path/../libhrd/ &
        echo finish $VARI
done
wait
for VARIABLE in "${pass_others[@]}"
