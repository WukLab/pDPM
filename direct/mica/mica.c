...
int mica_read_one_key_post(unsigned long long tar_key, int tar_mem, struct hrd_ctrl_blk** mem_cb,
                   void *mitsume_space, struct ibv_mr *mitsume_mr,
                   struct hrd_qp_attr ***mr_qp_list, int wrkr_lid)
{
...
int mica_write_one_key_post(unsigned long long tar_key, int tar_mem, struct hrd_ctrl_blk** mem_cb, 
                   void *mitsume_space, struct ibv_mr *mitsume_mr,
                   struct hrd_qp_attr ***mr_qp_list, int wrkr_lid)
{
        struct ibv_sge test_sge;
...
void mica_poll(struct hrd_ctrl_blk** mem_cb, int tar_mem, int target_number)
{
                        ...
                        int wrkr_lid)
{
        ...
        int finish_flag = 0;
        int ret;
        int tarmem_counter[MICA_NUM_MEMORY];
        int tar_mem;
        int per_replication;
        ...
        while(!finish_flag)
        {
                ...
                for(i=0;i<n;i++)
                {
                        ...
                        {
                                ret = mica_read_one_key_post(tar_key, tar_mem, mem_cb, mitsume_space_list[i], mitsume_mr_list[i], mr_qp_list, wrkr_lid);
                                tarmem_counter[ret]++;
                ...
                {
                        if(tarmem_counter[j])
                                mica_poll(mem_cb, j, tarmem_counter[j]);
                }
                memset(tarmem_counter, 0, sizeof(int) * MICA_NUM_MEMORY);
                ...
                for(i=0;i<n;i++)
                {
                        ...
                        if(original_process[i] == GET_LOCK && op[i]->opcode == MICA_OP_PUT)
                        {
                                for(per_replication=0;per_replication<MICA_NUM_REPLICATION;per_replication++)
                                {
                                        ...
                                        ret = mica_write_one_key_post(tar_key, tar_mem, mem_cb, mitsume_space_list[i], mitsume_mr_list[i], backupmr_qp_list, wrkr_lid);
                                        tarmem_counter[ret]++;
                                        original_process[i] = GENERATE_REQUEST;
                                }
                        }
                ...
                for(i=0;i<n;i++)
                {
                        ...
                        if(original_process[i] == GET_LOCK && op[i]->opcode == MICA_OP_PUT)
                        {
                                ...
                                {
                                        tar_mem = (tar_key+per_replication) % MICA_NUM_MEMORY;
                                        ret = mica_write_one_key_post(tar_key, tar_mem, mem_cb, mitsume_space_list[i], mitsume_mr_list[i], mr_qp_list, wrkr_lid);
                                        tarmem_counter[ret]++;
                                        original_process[i] = GENERATE_REQUEST;
                                }
                        }
...
void mica_batch_op(struct mica_kv* kv, int n, struct mica_op** op,
  ...
  mitsume_process(op, n, mem_cb, mitsume_space_list, mitsume_mr_list, mr_qp_list, backupmr_qp_list, wrkr_lid);
  for (I = 0; I < n; I++) {
    if (kv_ptr[I] != NULL) {
